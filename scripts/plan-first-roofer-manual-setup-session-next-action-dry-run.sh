#!/usr/bin/env bash
echo "== FIRST ROOFER MANUAL SETUP SESSION NEXT ACTION DRY RUN =="
...
test -f docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_NEXT_ACTION.md || { echo "FAILED: missing session next action doc"; exit 1; }
test -f scripts/plan-first-roofer-manual-setup-session-next-action-dry-run.sh || { echo "FAILED: missing session next action wrapper"; exit 1; }
test -f backend/scripts/verify-roofer-dry-run-first-roofer-manual-setup-session-next-action-readonly.js || { echo "FAILED: missing session next action verifier"; exit 1; }
...
echo "MANUAL SETUP SESSION NEXT ACTION DRY RUN PASS"
