...
echo "== RoofLeadHQ Native Workflow Fixture Controlled Real Roofer Pilot Setup Completeness Gate Dry Run =="
echo "Mode: local fake-data review-only; planning-only; not-approved; non-executing."
echo "This wrapper validates committed controlled real roofer pilot setup completeness gate artifacts — it does NOT approve or execute activation."
...
echo "Setup evidence capture packet is completed upstream (source_of_truth_commit 0159faf referenced)."
echo "All 12 recommended setup steps remain not_captured (0 captured, 12 missing)."
...
echo "Controlled real roofer setup completeness status is incomplete."
...
echo "Setup completeness gate does not equal approval. Setup completeness no-go review does not equal approval."
...
echo "Controlled real roofer validation blocked until setup evidence is complete and separately approved."
...
node --check backend/scripts/verify-native-workflow-fixture-controlled-real-roofer-pilot-setup-completeness-gate-readonly.js
echo "PASS: Controlled real roofer pilot setup completeness gate verifier syntax check (node --check) succeeded."
...
echo "PASS: Native Workflow Fixture Controlled Real Roofer Pilot Setup Completeness Gate Dry Run wrapper passed."
