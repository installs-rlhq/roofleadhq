...
echo "== RoofLeadHQ First Roofer Estimate / Next-Step Readiness Command Packet Dry-Run =="
...
node --check backend/scripts/verify-first-roofer-estimate-next-step-readiness-command-packet-readonly.js
echo "PASS: verifier syntax check (node --check) succeeded."

# Run the dedicated estimate / next-step readiness command packet verifier
node backend/scripts/verify-first-roofer-estimate-next-step-readiness-command-packet-readonly.js
...
echo "PASS: First Roofer Estimate / Next-Step Readiness Command Packet dry-run wrapper passed."
