...
echo "== RoofLeadHQ First Roofer Manual Downstream Routing Command Packet Dry-Run =="
...
node --check backend/scripts/verify-first-roofer-manual-downstream-routing-command-packet-readonly.js
echo "PASS: verifier syntax check (node --check) succeeded."

# Run the dedicated manual downstream routing command packet verifier
node backend/scripts/verify-first-roofer-manual-downstream-routing-command-packet-readonly.js
...
echo "PASS: First Roofer Manual Downstream Routing Command Packet dry-run wrapper passed."
