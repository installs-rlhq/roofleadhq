...
echo "== RoofLeadHQ Build 240 — Vapi-Originated Validation Fresh Pre-Run Guard Dry-Run =="
echo "Mode: local-only. Read-only static verifier + Build 239 + Build 238 + Build 237 verifiers + Vapi smoke regression."
...
echo "Recorded outcome: fresh pre-run guard READY for the single, already-approved (Build 239) Vapi-originated synthetic/controlled webhook validation (Test Roofing Assistant only, sanitized capture only). Validation itself NOT executed by this build. Real call test + full payload processing remain gated."

echo ""
echo "== Syntax check (node --check) =="
node --check backend/scripts/verify-vapi-originated-validation-pre-run-guard-build-240-readonly.js
echo "PASS: Build 240 script syntax check succeeded."

echo ""
echo "== Build 240 read-only static verifier (non-mutating; no live HTTP; no Vapi-originated action) =="
...
echo ""
echo "PASS: Build 240 Vapi-originated validation fresh pre-run guard dry-run passed."
