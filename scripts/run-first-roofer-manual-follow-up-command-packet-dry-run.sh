...
echo "== RoofLeadHQ First Roofer Manual Follow-Up Command Packet Dry-Run =="
...
node --check backend/scripts/verify-first-roofer-manual-follow-up-command-packet-readonly.js
echo "PASS: verifier syntax check (node --check) succeeded."

# Run the dedicated manual follow-up command packet verifier
node backend/scripts/verify-first-roofer-manual-follow-up-command-packet-readonly.js
...
echo "PASS: First Roofer Manual Follow-Up Command Packet dry-run wrapper passed."
