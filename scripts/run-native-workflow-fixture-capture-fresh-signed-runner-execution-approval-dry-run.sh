...
echo "== RoofLeadHQ Native Workflow Fixture Capture Fresh Signed Runner-Execution Approval Dry Run =="
echo "Mode: local fresh signed runner-execution approval capture only; review-only; not fresh execution pre-run guard; not runner execution; not activation; non-executing."
echo "This wrapper validates committed fresh signed runner-execution approval capture packet artifacts — it does NOT run the runner, pass the fresh execution pre-run guard, invoke the exact approved command, or execute activation."
echo "No external calls, no credentials, no production data, no roofer/homeowner contact, no SMS/email/calls/calendar booking."
echo "source_of_truth_commit is 31019fb. Build 109 fresh runner-execution exact decision template packet (31019fb), Build 108 runner state wiring correction (77f2a00), and Build 107 blocked command evidence (4a618fa) are referenced."
...
echo "sms_email_calls_calendar_booking_approval_status is not_granted_by_this_packet_until_fresh_execution_pre_run_guard_passes."
...
echo "future_command_status is blocked_until_fresh_execution_pre_run_guard_passes."
echo "approved_for_activation_now is false. Next step is separate fresh execution pre-run guard, not execution."
echo "No source-of-truth check inside this wrapper (agent worktrees can be ahead/behind main during branch review)."
echo "Full aggregate regression remains available for milestone/high-risk review (Terminal 1)."

node --check backend/scripts/verify-native-workflow-fixture-capture-fresh-signed-runner-execution-approval-readonly.js
echo "PASS: fresh signed runner-execution capture verifier syntax check (node --check) succeeded."

node backend/scripts/verify-native-workflow-fixture-capture-fresh-signed-runner-execution-approval-readonly.js

echo "PASS: Native Workflow Fixture Capture Fresh Signed Runner-Execution Approval Dry Run wrapper passed."
echo "Note: This fresh signed runner-execution approval capture does NOT run the runner or pass the fresh execution pre-run guard."
echo "Note: Fresh signed runner-execution approval is captured for one-time scoped sandbox/test-mode run pending fresh pre-run guard only."
...
echo "Note: Next step is separate fresh execution pre-run guard, not execution."