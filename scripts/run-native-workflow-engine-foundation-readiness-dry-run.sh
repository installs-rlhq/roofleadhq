...
echo "== RoofLeadHQ Native Workflow Engine Foundation Readiness Packet Dry-Run =="
echo "Mode: read-only verification and native workflow engine foundation planning only"
echo "No schema changes, no production data reads, no production activation."
echo "No source-of-truth check inside this wrapper (agent worktrees can be ahead/behind main during branch review)."

node --check backend/scripts/verify-native-workflow-engine-foundation-readiness-readonly.js
...
echo "PASS: Native Workflow Engine Foundation Readiness Packet dry-run wrapper passed."
