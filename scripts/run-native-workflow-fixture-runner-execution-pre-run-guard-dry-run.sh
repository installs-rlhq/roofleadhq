...
echo "== RoofLeadHQ Native Workflow Fixture Runner-Execution Pre-Run Guard Dry Run =="
echo "Mode: local execution pre-run guard only; review-only; not runner execution; not activation; non-executing."
echo "This wrapper validates committed runner-execution pre-run guard packet artifacts — it does NOT run the runner, invoke the exact approved command, or execute activation."
echo "No external calls, no credentials, no production data, no roofer/homeowner contact, no SMS/email/calls/calendar booking."
echo "source_of_truth_commit is bb0bc14. Build 105 signed runner-execution approval capture packet (bb0bc14) is referenced and verified."
...
echo "external_sandbox_calls_approval_status is granted_scoped_test_mode_only (verified for future scoped run only; not performed by this packet)."
echo "credentials_access_approval_status is granted_scoped_test_mode_only_no_secret_logging (verified for future scoped run only; not performed by this packet)."
echo "test_account_use_approval_status is granted_scoped_test_accounts_only (verified for future scoped run only; not performed by this packet)."
...
echo "runner_command_path_status is created_fail_closed_pending_exact_approved_execution_command."
...
echo "approved_for_activation_now is false. Next step is exact approved runner command execution in Terminal 1 with no substitutions from /root/roofleadhq, or stop/review."
echo "No source-of-truth check inside this wrapper (agent worktrees can be ahead/behind main during branch review)."
echo "Full aggregate regression remains available for milestone/high-risk review (Terminal 1)."

node --check backend/scripts/verify-native-workflow-fixture-runner-execution-pre-run-guard-readonly.js
echo "PASS: runner-execution pre-run guard verifier syntax check (node --check) succeeded."

node backend/scripts/verify-native-workflow-fixture-runner-execution-pre-run-guard-readonly.js

echo "PASS: Native Workflow Fixture Runner-Execution Pre-Run Guard Dry Run wrapper passed."
echo "Note: This runner-execution pre-run guard does NOT run the runner or invoke the exact approved command."
echo "Note: Runner-execution pre-run guard passed for scoped one-time runner execution command review only."
...
echo "Note: Next step is exact approved runner command execution in Terminal 1 with no substitutions from /root/roofleadhq, or stop/review."