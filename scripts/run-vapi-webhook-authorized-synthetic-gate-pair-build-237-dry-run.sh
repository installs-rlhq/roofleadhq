...
echo "== RoofLeadHQ Build 237 — Vapi Webhook Authorized Synthetic Gate-Pair Dry-Run =="
echo "Mode: local-only. Read-only static verifier + Build 236 + Build 235 + Build 234 + Build 233 + Build 232 verifiers + Vapi smoke regression."
echo "No call. No deploy. No Railway var set. No Vapi/Twilio config change. No live mutating HTTP. No Supabase write. No SMS. No full payload pass. No real call test. No secret file read."
...
echo "Recorded outcome: auth gate-pair PASS (unauth POST -> 401 unauthorized; authorized x-vapi-webhook-secret POST -> 400 missing_required_field). Full payload pass + real call test NOT executed, NOT fabricated."

echo ""
echo "== Syntax check (node --check) =="
node --check backend/scripts/verify-vapi-webhook-authorized-synthetic-gate-pair-build-237-readonly.js
echo "PASS: Build 237 script syntax check succeeded."

echo ""
echo "== Build 237 read-only static verifier (non-mutating; no live HTTP) =="
