...
echo "== RoofLeadHQ Native Workflow Fixture State Model Plan Dry-Run =="
echo "Mode: read-only verification and native workflow fixture state model planning only"
echo "No schema changes, no production data reads, no production activation."
echo "No source-of-truth check inside this wrapper (agent worktrees can be ahead/behind main during branch review)."

node --check backend/scripts/verify-native-workflow-fixture-state-model-plan-readonly.js
...
echo "PASS: Native Workflow Fixture State Model Plan dry-run wrapper passed."
