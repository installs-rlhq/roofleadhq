# Jason-Operated Controlled Pilot Expansion One-Message Send — Command Runbook (Build 200, Option B, after destination validation)
...
**Context:** This is the **retry after destination validation**. The Build 198 approval was already
**consumed** by the failed Build 199 attempt (Twilio error 21211 / status 400). Build 200 captured a
corrected destination validation marker and a **fresh** signed one-message approval. This send runs
under that **fresh** approval — it is **not** a retry of the consumed Build 198 approval.
...
Jason performs in his own shell with his own live credentials and the corrected/validated destination.
Nothing in this repository performs the send.

## Preconditions (all must be true, confirmed by Jason)

- Corrected destination validation marker captured:
  `controlled-roofer-pilot-expansion-destination-validation-build-200.json`
...
- Fresh signed send approval captured:
  `controlled-roofer-pilot-expansion-final-send-approval-build-200-signed.json`
  (`is_fresh_signed_approval=true`, `is_retry_of_consumed_approval=false`,
  `authorizes_jason_operated_one_message_send=true`, `authorizes_build_environment_send=false`).
- Send-time preflight is
...
- The corrected/validated destination is the same consenting test identity (E.164), supplied **only**
...
The send uses the existing fail-closed one-message runner — no new send code is added by Build 200:
...
- `CONTROLLED_LIVE_SMS_TO_NUMBER`  *(the corrected/validated same consenting test-identity destination, E.164)*
...
- Hand the result back for a separate LOCAL-ONLY expansion-send closeout build (no live action),
  mirroring the Build 196 / Build 199 closeout pattern.
...
- This runs under the **fresh** Build 200 approval, not the consumed Build 198 approval.
