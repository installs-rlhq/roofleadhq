...
After fast-forward merging an agent branch into canonical main, do not treat full aggregate readiness as a required pre-push blocker while local HEAD is ahead of origin/main. Use targeted packet verifier + product quality gate + backend build + clean git status pre-push; then push/fetch/source-of-truth verification; then run full aggregate/backend build/source-of-truth verification. Cleanup should be idempotent by checking whether the worktree or local branch exists before removing/deleting.
