# Roofer Dry-Run — First Roofer Manual Setup Session Completion Lock

## Purpose

Records the internal founder/operator completion lock for the First Roofer Manual Setup Session dry-run chain.
...
Use HOLD when the chain is safe but needs a non-blocking clarification before the completion lock should be relied on.
