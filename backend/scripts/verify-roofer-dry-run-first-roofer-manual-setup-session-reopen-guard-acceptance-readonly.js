...
const requiredDocStrings = [
  ...
  'reopen guard has been reviewed',
  'preserved and frozen session chain remains protected from accidental mutation',
  'future reopen requires an explicit founder/operator decision',
...
const requiredWrapperStrings = [
  ...
  'verify-roofer-dry-run-first-roofer-manual-setup-session-reopen-guard-readonly.js',
...
const requiredIntegrationChecks = {
  ...
  'docs/FIRST_PAID_LAUNCH_VERIFIER_INDEX.md': [
    'verify-roofer-dry-run-first-roofer-manual-setup-session-reopen-guard-acceptance-readonly.js',
    'ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_REOPEN_GUARD_ACCEPTANCE.md',
    'accept-first-roofer-manual-setup-session-reopen-guard-dry-run.sh',
  ],
...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_REOPEN_GUARD_ACCEPTANCE.md');
const wrapper = read('scripts/accept-first-roofer-manual-setup-session-reopen-guard-dry-run.sh');

for (const value of requiredDocStrings) {
  assert(doc.includes(value), `Reopen guard acceptance doc missing required text: ${value}`);
}

for (const value of requiredWrapperStrings) {
  assert(wrapper.includes(value), `Reopen guard acceptance wrapper missing required command: ${value}`);
}

for (const value of forbiddenWrapperStrings) {
  assert(!wrapper.includes(value), `Reopen guard acceptance wrapper contains forbidden live/prod pattern: ${value}`);
}

for (const [file, requiredStrings] of Object.entries(requiredIntegrationChecks)) {
  const content = read(file);
  for (const value of requiredStrings) {
    assert(content.includes(value), `${file} missing reopen guard acceptance integration text: ${value}`);
  }
}

console.log('PASS: First roofer manual setup session reopen guard acceptance verifier passed.');
