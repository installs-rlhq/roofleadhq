...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_CLOSEOUT.md');
const wrapper = read('scripts/closeout-first-roofer-manual-setup-session-dry-run.sh');
...
[
  ...
  'internal only, founder/operator only, and session-closeout-only',
  'MANUAL SETUP SESSION CLOSEOUT PASS',
  'MANUAL SETUP SESSION CLOSEOUT HOLD',
  'MANUAL SETUP SESSION CLOSEOUT BLOCKED',
...
if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-closeout-readonly.js')) fail('aggregate missing session closeout verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_CLOSEOUT.md')) fail('index missing session closeout packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Closeout Packet')) fail('first-paid context missing session closeout packet');
if (!roofer.includes('First Roofer Manual Setup Session Closeout Packet')) fail('roofer context missing session closeout packet');

console.log('PASS: first-roofer manual setup session closeout verifier passed.');
