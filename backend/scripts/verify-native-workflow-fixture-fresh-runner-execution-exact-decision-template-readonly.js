...
const SOURCE_OF_TRUTH_COMMIT = '77f2a00';
...
const FRESH_RUNNER_EXECUTION_TEMPLATE_CHECKS = [
  'Build 103-108 upstream packets referenced and runner state wiring corrected_review_only',
  'runner command path exists executable and corrected fail-closed pending fresh exact execution decision',
  ...
  'future_command_status blocked until fresh runner execution approval captured',
];
...
const REQUIRED_ASSERTIONS = [
  ...
  'source_of_truth_commit_77f2a00_referenced',
  'build_108_runner_state_wiring_correction_referenced',
  'build_107_blocked_command_evidence_referenced',
  'build_106_pre_run_guard_referenced',
  'build_105_signed_approval_capture_referenced',
  'build_104_runner_execution_exact_approval_template_referenced',
  ...
  'future_command_status_blocked_until_fresh_runner_execution_approval_captured',
  ...
  'fresh_runner_execution_decision_template_dry_run_wrapper_present_and_safe',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_FRESH_RUNNER_EXECUTION_EXACT_DECISION_TEMPLATE.md',
  'NATIVE_WORKFLOW_FIXTURE_FRESH_RUNNER_EXECUTION_EXACT_DECISION_TEMPLATE_NO_GO_REVIEW.md',
  'run-native-workflow-fixture-fresh-runner-execution-exact-decision-template-dry-run.sh',
  'verify-native-workflow-fixture-fresh-runner-execution-exact-decision-template-readonly.js',
  'fresh-runner-execution-exact-decision-template.json',
  'Native Workflow Fixture Fresh Runner-Execution Exact Decision Template',
  'native workflow fixture fresh runner execution exact decision template',
  'fresh runner execution exact decision template',
];
...
passAssertion('source_of_truth_commit_77f2a00_referenced');
...
passAssertion('build_108_runner_state_wiring_correction_referenced');
...
passAssertion('build_107_blocked_command_evidence_referenced');
...
passAssertion('build_106_pre_run_guard_referenced');
...
passAssertion('build_105_signed_approval_capture_referenced');
...
passAssertion('build_104_runner_execution_exact_approval_template_referenced');
...
passAssertion('future_command_status_blocked_until_fresh_runner_execution_approval_captured');
...
mustHave(packetDoc, 'runner_command_path_status | corrected_fail_closed_pending_fresh_exact_execution_decision', 'packet doc');
...
mustHave(packetDoc, 'FRESH RUNNER EXECUTION TEMPLATE ONLY — NOT SIGNED — NOT APPROVED — DO NOT EXECUTE', 'packet doc');
mustHave(
  packetDoc,
...
if (!fs.existsSync(path.join(root, upstreamRunnerStateWiringCorrectionFixturePath))) {
  fail(`missing upstream runner state wiring correction fixture: ${upstreamRunnerStateWiringCorrectionFixturePath}`);
}
...
if (!fs.existsSync(path.join(root, upstreamPreRunGuardFixturePath))) {
  fail(`missing upstream pre-run guard fixture: ${upstreamPreRunGuardFixturePath}`);
}
...
if (!fs.existsSync(path.join(root, upstreamRunnerExecutionTemplateFixturePath))) {
  fail(`missing upstream runner execution template fixture: ${upstreamRunnerExecutionTemplateFixturePath}`);
}
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be 77f2a00');
}
...
if (fixture.runner_command_path_status !== 'corrected_fail_closed_pending_fresh_exact_execution_decision') {
...
if (fixture.future_command_status !== 'blocked_until_fresh_runner_execution_approval_captured') {
...
if (upstreamRunnerStateWiringCorrectionFixture.runner_state_wiring_correction_status !== 'corrected_review_only') {
  fail('upstream runner state wiring correction fixture runner_state_wiring_correction_status mismatch');
}
if (
  upstreamRunnerStateWiringCorrectionFixture.runner_command_path_status !==
  'corrected_fail_closed_pending_fresh_exact_execution_decision'
) {
  fail('upstream runner state wiring correction fixture runner_command_path_status mismatch');
}
...
mustHave(runnerScript, 'corrected_fail_closed_pending_fresh_exact_execution_decision', 'runner script');
...
mustHave(wrapper, 'local fresh runner-execution exact decision template only', 'wrapper mode');
...
mustHave(
  wrapper,
  'future_command_status is blocked_until_fresh_runner_execution_approval_captured',
  'wrapper mode',
);
mustHave(wrapper, 'Jason review/sign fresh exact runner-execution approval template', 'wrapper mode');
...
if (!isExecutable(wrapperPath)) {
  fail(`fresh runner execution decision template dry-run wrapper is not executable: ${wrapperPath}`);
}
passAssertion('fresh_runner_execution_decision_template_dry_run_wrapper_present_and_safe');
...
if (REQUIRED_ASSERTIONS.length !== 92) {
  fail(`REQUIRED_ASSERTIONS must contain 92 items (found ${REQUIRED_ASSERTIONS.length})`);
}

console.log(
  `PASS: Native Workflow Fixture Fresh Runner-Execution Exact Decision Template verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
