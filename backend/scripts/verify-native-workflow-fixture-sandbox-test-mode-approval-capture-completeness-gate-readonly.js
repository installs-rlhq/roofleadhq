...
const SOURCE_OF_TRUTH_COMMIT = 'f56340f';
const UPSTREAM_JASON_CAPTURE_COMMIT = '878fc77';
...
const RECOMMENDED_NEXT_STEP =
  'JASON_COMPLETE_SANDBOX_TEST_MODE_APPROVAL_CAPTURE_BEFORE_ACTIVATION_CONSIDERATION';
const EXACT_COMMAND =
...
const NO_GO_MISSING_ITEMS = [
  'exact signed Jason approval text',
  'approval timestamp',
  'approval scope',
  'services',
  'environment',
  ...
  'all 19 accepted exact values',
  'all 19 approved exact values',
];

const REQUIRED_ASSERTIONS = [
  'packet_doc_present',
  'no_go_review_doc_present',
  'jason_capture_doc_present',
  ...
  'upstream_jason_approval_capture_fixture_present',
  ...
  'source_of_truth_commit_f56340f_referenced',
  ...
  'jason_approval_capture_packet_status_completed',
  'approval_capture_status_not_captured',
  'jason_signed_approval_status_not_signed',
  'approval_capture_completeness_status_incomplete',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_SANDBOX_TEST_MODE_APPROVAL_CAPTURE_COMPLETENESS_GATE.md',
  'NATIVE_WORKFLOW_FIXTURE_SANDBOX_TEST_MODE_APPROVAL_CAPTURE_NO_GO_REVIEW.md',
  'run-native-workflow-fixture-sandbox-test-mode-approval-capture-completeness-gate-dry-run.sh',
  'verify-native-workflow-fixture-sandbox-test-mode-approval-capture-completeness-gate-readonly.js',
  'sandbox-test-mode-approval-capture-completeness-gate.json',
  'Native Workflow Fixture Sandbox/Test-Mode Approval Capture Completeness Gate',
  'native workflow fixture sandbox test mode approval capture completeness gate',
  'sandbox test mode approval capture completeness gate',
];
...
const jasonCaptureDoc = read(jasonCaptureDocPath);
...
passAssertion('jason_capture_doc_present');
...
passAssertion('source_of_truth_commit_f56340f_referenced');
...
mustHave(packetDoc, 'jason_approval_capture_packet_status | completed', 'packet doc');
passAssertion('jason_approval_capture_packet_status_completed');
...
mustHave(packetDoc, 'approval_capture_completeness_status | incomplete', 'packet doc');
passAssertion('approval_capture_completeness_status_incomplete');
...
mustHave(noGoReviewDoc, 'Recommended defaults are **not** approval', 'no-go review doc');
...
mustHave(
  noGoReviewDoc,
  'No future sandbox/test-mode activation command can even be considered',
  'no-go review doc',
);
passAssertion('gate_blocks_activation_consideration');
...
if (!fs.existsSync(path.join(root, jasonCaptureFixturePath))) {
  fail(`missing upstream jason approval capture fixture: ${jasonCaptureFixturePath}`);
}
passAssertion('upstream_jason_approval_capture_fixture_present');
...
const jasonCaptureFixture = readJson(jasonCaptureFixturePath);
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be f56340f');
}
if (fixture.packet_status !== 'review_only') {
  fail('fixture packet_status must be review_only');
}
if (fixture.review_status !== 'approval_capture_completeness_gate_review_only') {
  fail('fixture review_status must be approval_capture_completeness_gate_review_only');
}
if (fixture.approval_capture_completeness_status !== 'incomplete') {
  fail('fixture approval_capture_completeness_status must be incomplete');
}
if (fixture.approval_capture_gate_decision !== 'NO_GO') {
  fail('fixture approval_capture_gate_decision must be NO_GO');
}
if (fixture.approval_capture_completeness_gate_does_not_equal_approval !== true) {
  fail('fixture approval_capture_completeness_gate_does_not_equal_approval must be true');
}
if (fixture.no_go_review_does_not_equal_approval !== true) {
  fail('fixture no_go_review_does_not_equal_approval must be true');
}
if (fixture.jason_approval_capture_packet_status !== 'completed') {
  fail('fixture jason_approval_capture_packet_status must be completed');
}
...
const expectedNoGoKeys = [
  'exact_signed_jason_approval_text',
  'approval_timestamp',
  'approval_scope',
  'services',
  'environment',
  ...
  'expiry',
  'one_time_use_limitation',
  'all_19_accepted_exact_values',
  'all_19_approved_exact_values',
];
for (const key of expectedNoGoKeys) {
...
if (jasonCaptureFixture.source_of_truth_commit !== UPSTREAM_JASON_CAPTURE_COMMIT) {
  fail('upstream jason capture fixture source_of_truth_commit must be 878fc77');
}
if (jasonCaptureFixture.packet_status !== 'review_only') {
  fail('upstream jason capture fixture packet_status must be review_only');
}
if (jasonCaptureFixture.approval_capture_status !== 'not_captured') {
  fail('upstream jason capture fixture approval_capture_status must be not_captured');
}
if (jasonCaptureFixture.jason_signed_approval_status !== 'not_signed') {
  fail('upstream jason capture fixture jason_signed_approval_status must be not_signed');
}
passAssertion('upstream_jason_capture_packet_status_completed');
...
const proposalFixtureForUpstream = readJson(proposalFixturePath);
if (proposalFixtureForUpstream.source_of_truth_commit !== UPSTREAM_PROPOSAL_COMMIT) {
  fail('upstream proposal fixture source_of_truth_commit must be 0cceb00');
}
if (proposalFixtureForUpstream.recommended_exact_values_proposed_count !== 19) {
  fail('upstream proposal fixture recommended_exact_values_proposed_count must be 19');
}
if (!proposalFixtureForUpstream.recommended_exact_values) {
  fail('upstream proposal fixture recommended_exact_values missing');
}
for (const field of EXACT_VALUE_FIELDS) {
  const recommended = proposalFixtureForUpstream.recommended_exact_values[field];
...
if (
  ...
  fixture.evidence_chain_commits.length !== 24
) {
  fail('fixture evidence_chain_commits must contain 24 commits');
}
for (const commit of EVIDENCE_CHAIN_COMMITS) {
...
mustHave(wrapper, 'Approval capture completeness gate does not equal approval', 'wrapper mode');
...
mustHave(packetDoc, jasonCaptureDocPath, 'packet doc');
...
if (REQUIRED_ASSERTIONS.length !== 125) {
  fail(`REQUIRED_ASSERTIONS must contain 125 items (found ${REQUIRED_ASSERTIONS.length})`);
}

console.log(
  `PASS: Native Workflow Fixture Sandbox/Test-Mode Approval Capture Completeness Gate verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
