...
const docPath = "docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_OPERATOR_RUNBOOK.md";
const wrapperPath = "scripts/run-first-roofer-manual-setup-operator-runbook-dry-run.sh";
...
for (const file of [wrapperPath, rehearsalWrapperPath, qaWrapperPath, docPath, rehearsalDocPath, qaDocPath, rehearsalVerifierPath, qaVerifierPath]) {
  read(file);
  mustHave(doc, file, "operator runbook doc");
}
for (const phrase of [
  "operator-runbook-only and dry-run only",
  ...
  "Operator checklist"
]) mustHave(doc, phrase, "operator runbook doc");
for (const flag of [
...
]) mustHave(doc, flag, "operator runbook doc");
for (const phrase of [
  "MANUAL SETUP OPERATOR RUNBOOK PASS",
  "MANUAL SETUP OPERATOR RUNBOOK HOLD",
  "MANUAL SETUP OPERATOR RUNBOOK BLOCKED"
]) mustHave(doc, phrase, "operator runbook doc");
for (const phrase of [
...
]) mustHave(wrapper, phrase, "operator runbook wrapper");
for (const unsafe of [
  ...
  "fetch(\"https://",
  "fetch('https://"
]) mustNotHave(wrapper, unsafe, "operator runbook wrapper");
console.log("PASS: first roofer manual setup operator runbook is present and dry-run only.");
