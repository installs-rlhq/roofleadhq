...
const setupEvidenceCaptureDocPath =
...
const SOURCE_OF_TRUTH_COMMIT = 'dbb30a7';
const UPSTREAM_COMPLETENESS_GATE_COMMIT = '0159faf';
...
const REQUIRED_ASSERTIONS = [
  ...
  'setup_evidence_capture_doc_present',
  ...
  'source_of_truth_commit_dbb30a7_referenced',
  ...
  'limited_validation_no_go_review_does_not_equal_approval',
  ...
  'controlled_real_roofer_limited_validation_status_incomplete',
  ...
  'all_5_scenario_ids_present',
  'all_5_scenarios_evidence_capture_status_not_captured',
  'all_5_scenarios_all_15_evidence_fields_blank',
  ...
  'packet_does_not_approve_controlled_real_roofer_validation',
  'packet_live_activation_remains_not_granted',
  'packet_blocks_controlled_real_roofer_limited_validation_execution',
  'packet_blocks_real_roofer_onboarding_contact',
  'controlled_real_roofer_validation_blocked_until_sandbox_and_setup_evidence_complete',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_CONTROLLED_REAL_ROOFER_LIMITED_VALIDATION_EVIDENCE_CAPTURE_PACKET.md',
  'NATIVE_WORKFLOW_FIXTURE_CONTROLLED_REAL_ROOFER_LIMITED_VALIDATION_EVIDENCE_TEMPLATE.md',
  'NATIVE_WORKFLOW_FIXTURE_CONTROLLED_REAL_ROOFER_LIMITED_VALIDATION_NO_GO_REVIEW.md',
  'run-native-workflow-fixture-controlled-real-roofer-limited-validation-evidence-capture-packet-dry-run.sh',
  'verify-native-workflow-fixture-controlled-real-roofer-limited-validation-evidence-capture-packet-readonly.js',
  'controlled-real-roofer-limited-validation-evidence-capture-packet.json',
  'Native Workflow Fixture Controlled Real Roofer Limited Validation Evidence Capture Packet',
  'native workflow fixture controlled real roofer limited validation evidence capture packet',
  'controlled real roofer limited validation evidence capture packet',
];
...
passAssertion('setup_evidence_capture_doc_present');
...
passAssertion('source_of_truth_commit_dbb30a7_referenced');
...
mustHave(packetDoc, 'controlled_real_roofer_pilot_setup_completeness_gate_status | completed', 'packet doc');
...
mustHave(evidenceTemplateDoc, 'Limited validation evidence template does **not** equal approval', 'evidence template doc');
passAssertion('limited_validation_evidence_template_does_not_equal_approval');

mustHave(packetDoc, 'Limited validation no-go review does **not** equal approval', 'packet doc');
mustHave(noGoReviewDoc, 'Limited validation no-go review does **not** equal approval', 'no-go review doc');
passAssertion('limited_validation_no_go_review_does_not_equal_approval');
...
mustHave(packetDoc, 'controlled_real_roofer_limited_validation_status | incomplete', 'packet doc');
passAssertion('controlled_real_roofer_limited_validation_status_incomplete');
...
passAssertion('all_5_scenario_ids_present');
...
passAssertion('controlled_real_roofer_validation_blocked_until_sandbox_and_setup_evidence_complete');

passAssertion('structured_fixture_present');
const fixture = readJson(fixturePath);

if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be dbb30a7');
}
...
if (fixture.controlled_real_roofer_limited_validation_status !== 'incomplete') {
  fail('fixture controlled_real_roofer_limited_validation_status must be incomplete');
}
...
if (fixture.limited_validation_no_go_review_does_not_equal_approval !== true) {
  fail('fixture limited_validation_no_go_review_does_not_equal_approval must be true');
}
...
if (
  !Array.isArray(fixture.controlled_real_roofer_limited_validation_scenarios) ||
  fixture.controlled_real_roofer_limited_validation_scenarios.length !== 5
) {
  fail('fixture controlled_real_roofer_limited_validation_scenarios must contain 5 scenarios');
}
for (const scenario of fixture.controlled_real_roofer_limited_validation_scenarios) {
...
passAssertion('all_5_scenarios_evidence_capture_status_not_captured');
passAssertion('all_5_scenarios_all_15_evidence_fields_blank');
...
mustHave(packetDoc, 'This packet does **not** approve sandbox/test-mode activation', 'packet doc');
passAssertion('packet_does_not_approve_live_activation');
mustHave(packetDoc, 'Live activation remains **not granted**', 'packet doc');
passAssertion('packet_does_not_approve_controlled_real_roofer_validation');
...
passAssertion('packet_blocks_controlled_real_roofer_limited_validation_execution');
...
if (
  ...
  fixture.evidence_chain_commits.length !== 29
) {
  fail('fixture evidence_chain_commits must contain 29 commits');
}
for (const commit of EVIDENCE_CHAIN_COMMITS) {
...
mustHave(wrapper, 'Limited validation evidence capture packet does not equal approval', 'wrapper mode');
mustHave(wrapper, 'Limited validation evidence template does not equal approval', 'wrapper mode');
mustHave(wrapper, 'Limited validation no-go review does not equal approval', 'wrapper mode');
...
mustHave(
  wrapper,
  'Controlled real roofer validation blocked until sandbox/test-mode evidence and setup evidence are complete and separately approved',
  'wrapper mode',
);
...
mustHave(packetDoc, setupEvidenceCaptureDocPath, 'packet doc');
...
if (REQUIRED_ASSERTIONS.length !== 131) {
  fail(`REQUIRED_ASSERTIONS must contain 131 items (found ${REQUIRED_ASSERTIONS.length})`);
}

console.log(
  `PASS: Native Workflow Fixture Controlled Real Roofer Limited Validation Evidence Capture Packet verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
