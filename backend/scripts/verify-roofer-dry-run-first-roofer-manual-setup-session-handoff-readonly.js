...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_HANDOFF.md');
const wrapper = read('scripts/handoff-first-roofer-manual-setup-session-dry-run.sh');
...
[
  ...
  'internal only, founder/operator only, and session-handoff-only',
  'MANUAL SETUP SESSION HANDOFF PASS',
  'MANUAL SETUP SESSION HANDOFF HOLD',
  'MANUAL SETUP SESSION HANDOFF BLOCKED',
...
[
  ...
  'verify-roofer-dry-run-first-roofer-manual-setup-session-closeout-readonly.js',
...
if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-handoff-readonly.js')) fail('aggregate missing session handoff verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_HANDOFF.md')) fail('index missing session handoff packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Handoff Packet')) fail('first-paid context missing session handoff packet');
if (!roofer.includes('First Roofer Manual Setup Session Handoff Packet')) fail('roofer context missing session handoff packet');

console.log('PASS: first-roofer manual setup session handoff verifier passed.');
