...
const SOURCE_OF_TRUTH_COMMIT = '9106d8f';
...
const APPROVED_EXACT_VALUES = {
  ...
  exact_command: EXACT_APPROVED_COMMAND,
  exact_working_directory: EXACT_APPROVED_WORKING_DIRECTORY,
  exact_credentials_env_api_webhook_boundary:
...
const WRAPPER_CORRECTION_CHECKS = [
  'source-of-truth HEAD equals approved HEAD',
  'exact signed Jason approval is captured upstream',
  'prior pre-run guard pass is referenced',
  'missing command path is documented',
  'similar scripts are not treated as approved substitutes',
  'exact approved command wrapper path is materialized',
  'corrected wrapper is executable',
  'corrected wrapper is local dry-run only',
  'all 19 exact values remain accepted and approved',
  'full safety state is demo_ready_with_live_automation_disabled',
];
...
const REQUIRED_ASSERTIONS = [
  ...
  'corrected_approved_command_wrapper_present',
  'corrected_approved_command_wrapper_executable',
  'source_of_truth_commit_9106d8f_referenced',
  'signed_approval_capture_packet_referenced',
  'prior_pre_run_guard_pass_referenced',
  'missing_command_path_detected_documented',
  'similar_scripts_not_approved_substitutes_documented',
  'exact_approved_command_path_materialized_documented',
  ...
  'correction_status_approved_command_wrapper_path_materialized',
  'missing_command_path_detected_true',
  'exact_approved_command_path_materialized_true',
  'exact_approved_command_byte_for_byte',
  'exact_approved_working_directory_byte_for_byte',
  'wrapper_correction_does_not_execute_approved_command_as_live_or_external_run',
  'command_execution_status_not_run_by_this_packet',
  'approved_for_activation_now_false',
  'future_command_status_ready_for_exact_approved_command_review_after_wrapper_correction_closeout',
  ...
  'wrapper_correction_checks_all_10_checks_present',
  'wrapper_correction_checks_all_10_checks_passed',
  'next_step_exact_approved_command_review_after_canonical_closeout',
  ...
  'correction_packet_dry_run_wrapper_present_and_safe',
  'corrected_approved_command_wrapper_safe',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_APPROVED_COMMAND_WRAPPER_CORRECTION_PACKET.md',
  'NATIVE_WORKFLOW_FIXTURE_APPROVED_COMMAND_WRAPPER_CORRECTION_PACKET_NO_GO_REVIEW.md',
  'run-native-workflow-fixture-approved-command-wrapper-correction-packet-dry-run.sh',
  'verify-native-workflow-fixture-approved-command-wrapper-correction-packet-readonly.js',
  'approved-command-wrapper-correction-packet.json',
  'Native Workflow Fixture Approved Command Wrapper Correction Packet',
  'native workflow fixture approved command wrapper correction',
  'approved command wrapper correction',
];
...
const correctedWrapper = read(correctedApprovedCommandWrapperPath);
...
mustHave(
  packetDoc,
  'fake-data/local-only/read-only/dry-run-only/review-only/wrapper-correction-only/non-executing',
  'packet doc',
);
mustHave(
  noGoReviewDoc,
...
passAssertion('source_of_truth_commit_9106d8f_referenced');
...
mustHave(packetDoc, 'missing_command_path_detected | true', 'packet doc');
mustHave(packetDoc, 'exact_approved_command_path_materialized | true', 'packet doc');
mustHave(packetDoc, 'correction_status | approved_command_wrapper_path_materialized', 'packet doc');
mustHave(packetDoc, 'not approved substitutes', 'packet doc');
mustHave(packetDoc, similarScriptCompletenessGate, 'packet doc');
mustHave(packetDoc, similarScriptEvidenceCapture, 'packet doc');
mustHave(noGoReviewDoc, 'NOT approved substitutes', 'no-go review');
mustHave(noGoReviewDoc, 'NOT_APPROVED_SUBSTITUTE', 'no-go review');
passAssertion('missing_command_path_detected_documented');
passAssertion('similar_scripts_not_approved_substitutes_documented');
passAssertion('exact_approved_command_path_materialized_documented');
...
passAssertion('correction_status_approved_command_wrapper_path_materialized');
passAssertion('missing_command_path_detected_true');
passAssertion('exact_approved_command_path_materialized_true');
...
passAssertion('wrapper_correction_does_not_execute_approved_command_as_live_or_external_run');
...
for (const check of WRAPPER_CORRECTION_CHECKS) {
  mustHave(packetDoc, check, 'packet doc wrapper correction checks');
  mustHave(packetDoc, `| ${check} | passed |`, 'packet doc wrapper correction check status');
}
passAssertion('wrapper_correction_checks_all_10_checks_present');
passAssertion('wrapper_correction_checks_all_10_checks_passed');
...
passAssertion('next_step_exact_approved_command_review_after_canonical_closeout');
...
if (!fs.existsSync(path.join(root, correctedApprovedCommandWrapperPath))) {
  fail(`missing corrected approved command wrapper: ${correctedApprovedCommandWrapperPath}`);
}
passAssertion('corrected_approved_command_wrapper_present');

if (!isExecutable(correctedApprovedCommandWrapperPath)) {
  fail(`corrected approved command wrapper is not executable: ${correctedApprovedCommandWrapperPath}`);
}
passAssertion('corrected_approved_command_wrapper_executable');
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be 9106d8f');
}
if (fixture.correction_status !== 'approved_command_wrapper_path_materialized') {
  fail('fixture correction_status mismatch');
}
if (fixture.missing_command_path_detected !== true) {
  fail('fixture missing_command_path_detected must be true');
}
if (fixture.exact_approved_command_path_materialized !== true) {
  fail('fixture exact_approved_command_path_materialized must be true');
}
...
if (fixture.corrected_approved_command_wrapper_path !== correctedApprovedCommandWrapperPath) {
  fail('fixture corrected_approved_command_wrapper_path mismatch');
}
if (
  fixture.future_command_status !==
  'ready_for_exact_approved_command_review_after_wrapper_correction_closeout'
) {
...
if (fixture.wrapper_correction_does_not_execute_approved_command_as_live_or_external_run !== true) {
  fail('fixture wrapper_correction_does_not_execute_approved_command_as_live_or_external_run must be true');
}
...
if (!Array.isArray(fixture.wrapper_correction_checks) || fixture.wrapper_correction_checks.length !== 10) {
  fail('fixture wrapper_correction_checks must contain 10 checks');
}
for (let i = 0; i < WRAPPER_CORRECTION_CHECKS.length; i += 1) {
  const check = fixture.wrapper_correction_checks[i];
  if (check.check !== i + 1) {
    fail(`fixture wrapper_correction_checks[${i}].check must be ${i + 1}`);
  }
  if (check.name !== WRAPPER_CORRECTION_CHECKS[i]) {
    fail(`fixture wrapper_correction_checks[${i}].name mismatch`);
  }
  if (check.status !== 'passed') {
    fail(`fixture wrapper_correction_checks[${i}].status must be passed`);
  }
}
...
if (
  ...
  fixture.evidence_chain_commits.length !== 38
) {
  fail('fixture evidence_chain_commits must contain 38 commits');
}
for (const commit of EVIDENCE_CHAIN_COMMITS) {
...
for (const pattern of UNSAFE_PATTERNS) {
  ...
  if (pattern.test(wrapper)) fail(`unsafe pattern ${pattern} found in correction packet wrapper`);
  if (pattern.test(correctedWrapper)) fail(`unsafe pattern ${pattern} found in corrected approved command wrapper`);
}
...
mustHave(verifierIndex, correctedApprovedCommandWrapperPath, 'verifier index');
...
mustHave(wrapper, 'local wrapper-correction only', 'wrapper mode');
...
mustNotHave(wrapper, EXACT_APPROVED_COMMAND, 'correction packet wrapper must not execute approved command');
passAssertion('correction_packet_dry_run_wrapper_present_and_safe');

mustHave(correctedWrapper, '#!/usr/bin/env bash', 'corrected wrapper shebang');
mustHave(correctedWrapper, 'set -eo pipefail', 'corrected wrapper strict mode');
mustHave(correctedWrapper, 'EXACT APPROVED COMMAND PATH', 'corrected wrapper');
mustHave(correctedWrapper, EXACT_APPROVED_COMMAND, 'corrected wrapper');
mustHave(correctedWrapper, EXACT_APPROVED_WORKING_DIRECTORY, 'corrected wrapper');
mustHave(correctedWrapper, 'node --check', 'corrected wrapper syntax checks');
mustHave(correctedWrapper, 'does NOT itself equal new approval', 'corrected wrapper');
mustHave(correctedWrapper, 'Any deviation requires new explicit Jason approval', 'corrected wrapper');
mustHave(correctedWrapper, 'No external calls', 'corrected wrapper');
mustHave(correctedWrapper, 'no credentials', 'corrected wrapper');
mustHave(correctedWrapper, 'no production data', 'corrected wrapper');
mustHave(correctedWrapper, 'no roofer/homeowner contact', 'corrected wrapper');
mustHave(correctedWrapper, 'no SMS/email/calls/calendar booking', 'corrected wrapper');
mustHave(
  correctedWrapper,
...
mustNotHave(correctedWrapper, 'verify-source-of-truth.sh', 'corrected wrapper');
mustNotHave(correctedWrapper, 'fetch(', 'corrected wrapper');
passAssertion('corrected_approved_command_wrapper_safe');
...
mustHave(packetDoc, correctedApprovedCommandWrapperPath, 'packet doc');
mustHave(packetDoc, noGoReviewDocPath, 'packet doc');

if (REQUIRED_ASSERTIONS.length !== 64) {
  fail(`REQUIRED_ASSERTIONS must contain 64 items (found ${REQUIRED_ASSERTIONS.length})`);
}

console.log(
  `PASS: Native Workflow Fixture Approved Command Wrapper Correction Packet verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
