...
const SOURCE_OF_TRUTH_COMMIT = '7f375a4';
...
const RECOMMENDED_NEXT_STEP = 'JASON_REVIEW_SANDBOX_TEST_MODE_APPROVAL_REQUEST_READY_PACKET';
const EXACT_COMMAND =
...
const REQUIRED_ASSERTIONS = [
  ...
  'source_of_truth_commit_7f375a4_referenced',
  ...
  'approval_request_ready_status_template_only_not_granted',
  'p0_blockers_count_0',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_SANDBOX_TEST_MODE_APPROVAL_REQUEST_READY_PACKET.md',
  'NATIVE_WORKFLOW_FIXTURE_SANDBOX_TEST_MODE_FINAL_JASON_APPROVAL_STATEMENT_TEMPLATE.md',
  'run-native-workflow-fixture-sandbox-test-mode-approval-request-ready-packet-dry-run.sh',
  'verify-native-workflow-fixture-sandbox-test-mode-approval-request-ready-packet-readonly.js',
  'sandbox-test-mode-approval-request-ready-packet.json',
  'Native Workflow Fixture Sandbox/Test-Mode Approval Request Ready Packet',
  'native workflow fixture sandbox test mode approval request ready packet',
  'sandbox test mode approval request ready packet',
];
...
for (const doc of [packetDoc, templateDoc]) {
  mustHave(doc, SOURCE_OF_TRUTH_COMMIT, 'packet doc');
}
passAssertion('source_of_truth_commit_7f375a4_referenced');
...
passAssertion('approval_request_ready_status_template_only_not_granted');
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be 7f375a4');
}
if (fixture.packet_status !== 'review_only') {
  fail('fixture packet_status must be review_only');
}
if (fixture.review_status !== 'approval_request_ready_review_only') {
  fail('fixture review_status must be approval_request_ready_review_only');
}
if (fixture.approval_request_ready_status !== 'template_only_not_granted') {
  fail('fixture approval_request_ready_status must be template_only_not_granted');
}
...
if (
  ...
  fixture.evidence_chain_commits.length !== 22
) {
  fail('fixture evidence_chain_commits must contain 22 commits');
}
for (const commit of EVIDENCE_CHAIN_COMMITS) {
...
console.log(
  `PASS: Native Workflow Fixture Sandbox/Test-Mode Approval Request Ready Packet verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
