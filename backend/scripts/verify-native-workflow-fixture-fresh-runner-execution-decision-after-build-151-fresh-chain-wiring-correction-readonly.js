...
const packetDocPath = 'docs/NATIVE_WORKFLOW_FIXTURE_FRESH_RUNNER_EXECUTION_DECISION_AFTER_BUILD_151_FRESH_CHAIN_WIRING_CORRECTION.md';
const fixturePath = 'backend/fixtures/native-workflow-demo-roofer/fresh-runner-execution-decision-after-build-151-fresh-chain-wiring-correction.json';
const wrapperPath = 'scripts/run-native-workflow-fixture-fresh-runner-execution-decision-after-build-151-fresh-chain-wiring-correction-dry-run.sh';
...
mustHave(doc, 'Native Workflow Fixture Fresh Runner Execution Decision After Build 151 Fresh Chain Wiring Correction', 'packet doc');
mustHave(doc, 'source_of_truth_commit | ae2a380', 'packet doc');
mustHave(doc, 'fresh_runner_execution_decision_template_gate_decision | NO_GO', 'packet doc');
mustHave(doc, 'approval_scope | fresh_run_actual_external_sandbox_30_scenario_validation_once_only_after_build_151_fresh_chain_wiring_correction', 'packet doc');
mustHave(doc, 'fresh_runner_execution_approval_capture_status | not_captured', 'packet doc');
mustHave(doc, 'fresh_runner_execution_jason_signed_approval_status | not_signed', 'packet doc');
mustHave(doc, 'fresh_runner_execution_exact_values_accepted_count | 0', 'packet doc');
mustHave(doc, 'fresh_runner_execution_exact_values_approved_count | 0', 'packet doc');
mustHave(doc, 'fresh_execution_pre_run_guard_status | not_passed', 'packet doc');
mustHave(doc, 'future_command_status | blocked_until_fresh_runner_execution_approval_captured_after_build_151_fresh_chain_wiring_correction', 'packet doc');
mustHave(doc, 'does **not** approve runner execution', 'packet doc');
mustHave(doc, 'does **not** invoke the runner', 'packet doc');

expectEq(fixture, 'source_of_truth_commit', 'ae2a380');
...
expectEq(fixture, 'future_command_status', 'blocked_until_fresh_runner_execution_approval_captured_after_build_151_fresh_chain_wiring_correction');
...
expectEq(fixture, 'prior_build_152_consumed_attempt_status', 'consumed');
expectEq(fixture, 'fresh_runner_execution_approval_required_after_build_151_fresh_chain_wiring_correction', true);
expectEq(fixture, 'fresh_execution_pre_run_guard_required_after_build_151_fresh_chain_wiring_correction', true);
expectEq(fixture, 'prior_fresh_runner_execution_approval_reusable_after_build_151_fresh_chain_wiring_correction', false);
expectEq(fixture, 'prior_fresh_execution_pre_run_guard_reusable_after_build_151_fresh_chain_wiring_correction', false);
...
mustHave(wrapper, 'node backend/scripts/verify-native-workflow-fixture-fresh-runner-execution-decision-after-build-151-fresh-chain-wiring-correction-readonly.js', 'wrapper');

for (const text of [aggregate, index, contextFirstPaid, contextAgent, guide, safeReadiness]) {
  if (!text.includes('Native Workflow Fixture Fresh Runner Execution Decision After Build 151 Fresh Chain Wiring Correction') || !text.includes('native workflow fixture fresh runner execution decision after build 151 fresh chain wiring correction')) {
...
const forbidden = [
  ...
  'fresh_runner_execution_approval_status: granted'
];
...
console.log('PASS: Native Workflow Fixture Fresh Runner Execution Decision After Build 151 Fresh Chain Wiring Correction verified (' + passCount + ' assertions).');
