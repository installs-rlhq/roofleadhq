...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_REOPEN_GUARD_FINAL_LOCK_ACCEPTANCE_PRESERVATION_SNAPSHOT_OPERATOR_HANDOFF_FREEZE_ARCHIVE.md');
const wrapper = read('scripts/archive-first-roofer-manual-setup-session-reopen-guard-final-lock-acceptance-preservation-snapshot-operator-handoff-freeze-dry-run.sh');
...
[
  ...
  'internal only, founder/operator only, and session-archive-only',
...
[
  ...
  'verify-roofer-dry-run-first-roofer-manual-setup-session-reopen-guard-final-lock-acceptance-preservation-snapshot-operator-handoff-freeze-archive-readonly.js',
  'scripts/check-production-gates.sh',
  'scripts/verify-safe-readiness.sh'
].forEach(s => { if (!wrapper.includes(s)) fail(`wrapper missing: ${s}`); });

if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-reopen-guard-final-lock-acceptance-preservation-snapshot-operator-handoff-freeze-archive-readonly.js')) fail('aggregate missing reopen guard final lock acceptance preservation snapshot operator handoff freeze archive verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_REOPEN_GUARD_FINAL_LOCK_ACCEPTANCE_PRESERVATION_SNAPSHOT_OPERATOR_HANDOFF_FREEZE_ARCHIVE.md')) fail('index missing reopen guard final lock acceptance preservation snapshot operator handoff freeze archive packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Reopen Guard Final Lock Acceptance Preservation Snapshot Operator Handoff Freeze Archive Packet')) fail('first-paid context missing reopen guard final lock acceptance preservation snapshot operator handoff freeze archive packet');
if (!roofer.includes('First Roofer Manual Setup Session Reopen Guard Final Lock Acceptance Preservation Snapshot Operator Handoff Freeze Archive Packet')) fail('roofer context missing reopen guard final lock acceptance preservation snapshot operator handoff freeze archive packet');

console.log('PASS: first-roofer manual setup reopen guard final lock acceptance preservation snapshot operator handoff freeze archive verifier passed.');
