...
const SOURCE_OF_TRUTH_COMMIT = '67393ed';
...
const RUNNER_EXECUTION_CAPTURE_CHECKS = [
  'Build 104 runner-execution exact approval template packet referenced upstream',
  'Build 103 runner scaffolding build referenced upstream built_review_only',
  ...
  'runner execution signed approval captured and signed',
  'all 24 runner execution exact values accepted and approved',
  ...
  'future_command_status blocked until runner execution pre-run guard passes',
];
...
const REQUIRED_ASSERTIONS = [
  ...
  'source_of_truth_commit_67393ed_referenced',
  ...
  'all_24_runner_execution_exact_values_present_and_approved',
  ...
  'runner_command_path_status_created_fail_closed_not_approved_to_run_until_execution_pre_run_guard_passes',
  ...
  'sms_email_calls_calendar_booking_approval_status_not_granted_by_this_packet_until_execution_pre_run_guard_passes',
  ...
  'execution_pre_run_guard_status_not_passed_by_this_packet',
  'future_command_status_blocked_until_runner_execution_pre_run_guard_passes',
  ...
  'packet_captures_signed_runner_execution_approval_evidence_only',
  'packet_does_not_run_runner',
  'packet_does_not_pass_execution_pre_run_guard',
  ...
  'next_step_execution_pre_run_guard_not_execution',
  'runner_execution_signed_approval_capture_checks_all_10_checks_present',
  'runner_execution_signed_approval_capture_checks_all_10_checks_passed',
  'demo_ready_with_live_automation_disabled_preserved',
  'full_safe_readiness_lane_preserved',
  'docs_and_context_wiring_present',
  'signed_runner_execution_capture_dry_run_wrapper_present_and_safe',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_CAPTURE_SIGNED_RUNNER_EXECUTION_APPROVAL.md',
  'NATIVE_WORKFLOW_FIXTURE_CAPTURE_SIGNED_RUNNER_EXECUTION_APPROVAL_NO_GO_REVIEW.md',
  'run-native-workflow-fixture-capture-signed-runner-execution-approval-dry-run.sh',
  'verify-native-workflow-fixture-capture-signed-runner-execution-approval-readonly.js',
  'capture-signed-runner-execution-approval.json',
  'Native Workflow Fixture Capture Signed Runner-Execution Approval',
  'native workflow fixture capture signed runner execution approval',
  'capture signed runner execution approval',
];
...
passAssertion('source_of_truth_commit_67393ed_referenced');
...
passAssertion('all_24_runner_execution_exact_values_present_and_approved');
...
mustHave(packetDoc, 'execution_pre_run_guard_status | not_passed_by_this_packet', 'packet doc');
...
passAssertion('execution_pre_run_guard_status_not_passed_by_this_packet');
...
passAssertion('future_command_status_blocked_until_runner_execution_pre_run_guard_passes');
...
passAssertion('packet_captures_signed_runner_execution_approval_evidence_only');
mustHave(
  packetDoc,
...
mustHave(packetDoc, 'This packet does **not** pass the execution pre-run guard by itself', 'packet doc');
passAssertion('packet_does_not_pass_execution_pre_run_guard');
...
mustHave(packetDoc, 'execution pre-run guard pass**, not execution', 'packet doc');
passAssertion('next_step_execution_pre_run_guard_not_execution');

mustHave(packetDoc, 'demo_ready_with_live_automation_disabled | preserved', 'packet doc');

for (const check of RUNNER_EXECUTION_CAPTURE_CHECKS) {
  mustHave(packetDoc, check, 'packet doc runner execution capture checks');
  mustHave(packetDoc, `| ${check} | passed |`, 'packet doc runner execution capture check status');
}
passAssertion('runner_execution_signed_approval_capture_checks_all_10_checks_present');
passAssertion('runner_execution_signed_approval_capture_checks_all_10_checks_passed');
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be 67393ed');
}
...
if (
  ...
  'not_granted_by_this_packet_until_execution_pre_run_guard_passes'
) {
...
if (fixture.execution_pre_run_guard_status !== 'not_passed_by_this_packet') {
  fail('fixture execution_pre_run_guard_status must be not_passed_by_this_packet');
}
if (fixture.future_command_status !== 'blocked_until_runner_execution_pre_run_guard_passes') {
...
if (
  !Array.isArray(fixture.runner_execution_signed_approval_capture_checks) ||
  fixture.runner_execution_signed_approval_capture_checks.length !== 10
) {
  fail('fixture runner_execution_signed_approval_capture_checks must contain 10 checks');
}
for (let i = 0; i < RUNNER_EXECUTION_CAPTURE_CHECKS.length; i += 1) {
  const check = fixture.runner_execution_signed_approval_capture_checks[i];
  if (check.check !== i + 1) {
    fail(`fixture runner_execution_signed_approval_capture_checks[${i}].check must be ${i + 1}`);
  }
  if (check.name !== RUNNER_EXECUTION_CAPTURE_CHECKS[i]) {
    fail(`fixture runner_execution_signed_approval_capture_checks[${i}].name mismatch`);
  }
  if (check.status !== 'passed') {
    fail(`fixture runner_execution_signed_approval_capture_checks[${i}].status must be passed`);
  }
}
...
if (
  upstreamScaffoldingBuildFixture.runner_command_path_status !== 'created_fail_closed_not_approved_to_run'
) {
...
mustHave(wrapper, 'local signed runner-execution approval capture only', 'wrapper mode');
mustHave(wrapper, 'not execution pre-run guard', 'wrapper mode');
...
mustHave(
  wrapper,
  'future_command_status is blocked_until_runner_execution_pre_run_guard_passes',
  'wrapper mode',
);
mustHave(wrapper, 'execution pre-run guard', 'wrapper mode');
...
if (!isExecutable(wrapperPath)) {
  fail(`signed runner-execution capture dry-run wrapper is not executable: ${wrapperPath}`);
}
passAssertion('signed_runner_execution_capture_dry_run_wrapper_present_and_safe');
...
console.log(
  `PASS: Native Workflow Fixture Capture Signed Runner-Execution Approval verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
