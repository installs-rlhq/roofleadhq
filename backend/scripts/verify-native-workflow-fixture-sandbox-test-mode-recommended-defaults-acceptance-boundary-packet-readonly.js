...
const SOURCE_OF_TRUTH_COMMIT = 'b6d852c';
...
const RECOMMENDED_NEXT_STEP =
  'JASON_REVIEW_SANDBOX_TEST_MODE_RECOMMENDED_DEFAULTS_ACCEPTANCE_BOUNDARY';
const EXACT_COMMAND =
...
const REQUIRED_ASSERTIONS = [
  ...
  'source_of_truth_commit_b6d852c_referenced',
  ...
  'acceptance_boundary_status_acceptance_boundary_review_only',
  'p0_blockers_count_0',
  ...
  'template_all_19_accept_edit_replace_rows_present',
  ...
  'boundary_does_not_approve_sandbox_test_mode_activation',
  'boundary_live_activation_remains_not_granted',
  'jason_future_acceptance_statement_not_signed_not_granted_template_only',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_SANDBOX_TEST_MODE_RECOMMENDED_DEFAULTS_ACCEPTANCE_BOUNDARY_PACKET.md',
  'NATIVE_WORKFLOW_FIXTURE_SANDBOX_TEST_MODE_RECOMMENDED_DEFAULTS_ACCEPT_EDIT_REPLACE_TEMPLATE.md',
  'run-native-workflow-fixture-sandbox-test-mode-recommended-defaults-acceptance-boundary-packet-dry-run.sh',
  'verify-native-workflow-fixture-sandbox-test-mode-recommended-defaults-acceptance-boundary-packet-readonly.js',
  'sandbox-test-mode-recommended-defaults-acceptance-boundary-packet.json',
  'Native Workflow Fixture Sandbox/Test-Mode Recommended Defaults Acceptance Boundary Packet',
  'native workflow fixture sandbox test mode recommended defaults acceptance boundary packet',
  'sandbox test mode recommended defaults acceptance boundary packet',
];
...
const combinedDocs = `${boundaryDoc}\n${templateDoc}\n${proposalDoc}`;

passAssertion('boundary_doc_present');
...
for (const doc of [boundaryDoc, templateDoc]) {
  mustHave(doc, SOURCE_OF_TRUTH_COMMIT, 'packet doc');
}
passAssertion('source_of_truth_commit_b6d852c_referenced');
...
mustHave(boundaryDoc, 'local_demo_e2e_evidence_chain_status | passed', 'boundary doc');
...
passAssertion('acceptance_boundary_status_acceptance_boundary_review_only');
mustHave(boundaryDoc, 'p0_blockers_count | 0', 'boundary doc');
passAssertion('p0_blockers_count_0');
mustHave(boundaryDoc, 'p1_polish_status | completed', 'boundary doc');
passAssertion('p1_polish_status_completed');
mustHave(boundaryDoc, 'p2_refinement_status | completed', 'boundary doc');
passAssertion('p2_refinement_status_completed');
mustHave(boundaryDoc, 'p3_planning_status | completed', 'boundary doc');
passAssertion('p3_planning_status_completed');

mustHave(boundaryDoc, 'Recommended defaults are **not** approval', 'boundary doc');
...
mustHave(boundaryDoc, 'exact_values_required_count | 19', 'boundary doc');
mustHave(boundaryDoc, 'recommended_exact_values_proposed_count | 19', 'boundary doc');
mustHave(boundaryDoc, 'accepted_exact_values_count | 0', 'boundary doc');
mustHave(boundaryDoc, 'accepted_exact_values_filled_count | 0', 'boundary doc');
mustHave(boundaryDoc, 'approved_exact_values_filled_count | 0', 'boundary doc');
mustHave(boundaryDoc, 'exact_values_filled_count | 0', 'boundary doc');
mustHave(boundaryDoc, 'all_exact_values_filled | false', 'boundary doc');
mustHave(boundaryDoc, 'completeness_status | incomplete', 'boundary doc');
...
mustHave(boundaryDoc, 'default_sandbox_test_mode_decision | HOLD', 'boundary doc');
mustHave(boundaryDoc, 'Default sandbox/test-mode decision remains **HOLD**', 'boundary doc');
passAssertion('default_sandbox_test_mode_decision_hold');

mustHave(boundaryDoc, 'approval_status | not_granted', 'boundary doc');
passAssertion('approval_status_not_granted');
mustHave(boundaryDoc, 'sandbox_test_mode_approval_status | not_granted', 'boundary doc');
passAssertion('sandbox_test_mode_approval_status_not_granted');
mustHave(boundaryDoc, 'live_activation_approval_status | not_granted', 'boundary doc');
...
passAssertion('template_all_19_accept_edit_replace_rows_present');
...
passAssertion('boundary_does_not_approve_sandbox_test_mode_activation');
mustHave(boundaryDoc, 'Live activation remains **not granted**', 'boundary doc');
passAssertion('boundary_live_activation_remains_not_granted');

mustHave(boundaryDoc, 'NOT SIGNED / NOT GRANTED / TEMPLATE ONLY', 'boundary doc');
mustHave(
  boundaryDoc,
  'jason_future_acceptance_statement_status | NOT_SIGNED_NOT_GRANTED_TEMPLATE_ONLY',
  'boundary doc',
);
mustHave(boundaryDoc, 'jason_future_acceptance_statement_signed | false', 'boundary doc');
mustHave(boundaryDoc, 'jason_future_acceptance_statement_granted | false', 'boundary doc');
passAssertion('jason_future_acceptance_statement_not_signed_not_granted_template_only');
...
mustHave(boundaryDoc, 'webhook', 'boundary doc');
mustHave(boundaryDoc, 'scheduler', 'boundary doc');
mustHave(boundaryDoc, 'billing', 'boundary doc');
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be b6d852c');
}
if (fixture.packet_status !== 'review_only') {
  fail('fixture packet_status must be review_only');
}
if (fixture.review_status !== 'acceptance_boundary_review_only') {
  fail('fixture review_status must be acceptance_boundary_review_only');
}
if (fixture.acceptance_boundary_status !== 'acceptance_boundary_review_only') {
  fail('fixture acceptance_boundary_status must be acceptance_boundary_review_only');
}
...
if (!fixture.jason_future_acceptance_statement) {
  fail('fixture jason_future_acceptance_statement missing');
}
if (fixture.jason_future_acceptance_statement.status !== 'NOT_SIGNED_NOT_GRANTED_TEMPLATE_ONLY') {
  fail('fixture jason_future_acceptance_statement.status mismatch');
}
if (fixture.jason_future_acceptance_statement.signed !== false) {
  fail('fixture jason_future_acceptance_statement.signed must be false');
}
if (fixture.jason_future_acceptance_statement.granted !== false) {
  fail('fixture jason_future_acceptance_statement.granted must be false');
}
if (fixture.jason_future_acceptance_statement.template_only !== true) {
  fail('fixture jason_future_acceptance_statement.template_only must be true');
}
...
if (
  ...
  fixture.evidence_chain_commits.length !== 21
) {
  fail('fixture evidence_chain_commits must contain 21 commits');
}
for (const commit of EVIDENCE_CHAIN_COMMITS) {
...
mustHave(verifierIndex, templateDocPath, 'verifier index');
...
mustHave(wrapper, 'Accept/edit/replace template does not equal approval', 'wrapper mode');
...
mustHave(wrapper, 'NOT SIGNED / NOT GRANTED / TEMPLATE ONLY', 'wrapper mode');
...
mustHave(boundaryDoc, templateDocPath, 'boundary doc');
mustHave(boundaryDoc, fixturePath, 'boundary doc');
mustHave(boundaryDoc, verifierPath, 'boundary doc');
mustHave(boundaryDoc, wrapperPath, 'boundary doc');
mustHave(boundaryDoc, proposalDocPath, 'boundary doc');
mustHave(boundaryDoc, worksheetDocPath, 'boundary doc');
mustHave(boundaryDoc, captureDraftDocPath, 'boundary doc');
mustHave(boundaryDoc, channelValidationDocPath, 'boundary doc');
mustHave(boundaryDoc, noGoStopDocPath, 'boundary doc');
...
console.log(
  `PASS: Native Workflow Fixture Sandbox/Test-Mode Recommended Defaults Acceptance Boundary Packet verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
