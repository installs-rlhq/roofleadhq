...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_NEXT_ACTION_ACCEPTANCE.md');
const wrapper = read('scripts/accept-first-roofer-manual-setup-session-next-action-dry-run.sh');
...
[
  ...
  'internal only, founder/operator only, and session-next-action-acceptance-only',
  'MANUAL SETUP SESSION NEXT ACTION ACCEPTANCE PASS',
  'MANUAL SETUP SESSION NEXT ACTION ACCEPTANCE HOLD',
  'MANUAL SETUP SESSION NEXT ACTION ACCEPTANCE BLOCKED',
...
[
  ...
  'verify-roofer-dry-run-first-roofer-manual-setup-session-next-action-acceptance-readonly.js',
  'scripts/check-production-gates.sh',
  'scripts/verify-safe-readiness.sh'
].forEach(s => { if (!wrapper.includes(s)) fail(`wrapper missing: ${s}`); });

if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-next-action-acceptance-readonly.js')) fail('aggregate missing session next action acceptance verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_NEXT_ACTION_ACCEPTANCE.md')) fail('index missing session next action acceptance packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Next Action Acceptance Packet')) fail('first-paid context missing session next action acceptance packet');
if (!roofer.includes('First Roofer Manual Setup Session Next Action Acceptance Packet')) fail('roofer context missing session next action acceptance packet');

console.log('PASS: first-roofer manual setup session next action acceptance verifier passed.');
