...
const REQUIRED_DOC_PHRASES = [
  'Purpose and Scope',
  'fake-data/local-only final review packet',
  'final review packet dry-run only',
  ...
  'Readiness Lock Summary',
  'Approval Request Summary',
  'Execution Runbook Summary',
  'Decision Ledger Summary',
  'Current Blocked Status Summary',
  'Approval Language Still Required',
  'Approval Signer Still Required',
  'Approval Timestamp Still Required',
...
const REQUIRED_FINAL_REVIEW_AREAS = [
  'Executive final review summary',
  'Evidence chain completeness',
  'First controlled launch decision ledger evidence',
  'First controlled launch execution runbook evidence',
...
const REQUIRED_ASSERTIONS = [
  ...
  'executive_final_review_summary_present',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_FIRST_CONTROLLED_LAUNCH_FINAL_REVIEW_PACKET_DRY_RUN.md',
  'run-native-workflow-fixture-first-controlled-launch-final-review-packet-dry-run.sh',
  'verify-native-workflow-fixture-first-controlled-launch-final-review-packet-dry-run-readonly.js',
  'run-native-workflow-fixture-first-controlled-launch-final-review-packet-dry-run.js',
  'Native Workflow Fixture First Controlled Launch Final Review Packet Dry Run',
  'native workflow fixture first controlled launch final review packet dry run',
  'first controlled launch final review packet dry run',
  'verify-safe-readiness-fast.sh',
];
...
passAssertion('first_controlled_launch_final_review_packet_doc_present');
...
passAssertion('final_review_packet_toc_present');
passAssertion('executive_final_review_summary_present');
...
passAssertion('readiness_lock_summary_present');
passAssertion('approval_request_summary_present');
passAssertion('execution_runbook_summary_present');
passAssertion('decision_ledger_summary_present');
passAssertion('approval_language_still_required_present');
passAssertion('approval_signer_still_required_present');
passAssertion('approval_timestamp_still_required_present');

passAssertion('scope_requested_present');
passAssertion('scope_excluded_present');
passAssertion('channel_final_review_present');
passAssertion('sms_final_review_present');
passAssertion('email_final_review_present');
passAssertion('call_vapi_final_review_present');
passAssertion('google_calendar_final_review_present');
passAssertion('csv_reporting_final_review_present');
passAssertion('crm_handoff_export_final_review_present');
passAssertion('lindy_bridge_final_review_present');
passAssertion('scheduler_dispatcher_final_review_present');
passAssertion('public_route_webhook_final_review_present');
passAssertion('supabase_persistence_final_review_present');
passAssertion('billing_payment_quote_boundary_blocked');
passAssertion('credential_env_final_review_present');
passAssertion('messaging_compliance_final_review_present');
passAssertion('data_boundary_pii_final_review_present');
passAssertion('audit_timeline_final_review_present');
passAssertion('owner_routing_final_review_present');
passAssertion('unresolved_blocker_register_present');
passAssertion('rollback_confirmation_placeholder_present');
passAssertion('post_approval_test_confirmation_placeholder_present');
passAssertion('final_reviewer_checklist_present');
...
for (const [label, scriptPath] of [
  ['runner', runnerPath],
  ['first controlled launch final review packet verifier', verifierPath],
]) {
...
if (
  output.first_controlled_launch_final_review_packet_dry_run !==
  'native_workflow_fixture_first_controlled_launch_final_review_packet_dry_run'
) {
  fail('first_controlled_launch_final_review_packet_dry_run marker is incorrect');
}

for (const section of REQUIRED_OUTPUT_SECTIONS) {
  if (!output[section]) fail(`output missing ${section}`);
}
console.log('PASS: top-level first controlled launch final review packet sections are present.');

if (
  !Array.isArray(output.first_controlled_launch_final_review_items) ||
  output.first_controlled_launch_final_review_items.length !== 36
) {
  fail('first_controlled_launch_final_review_items must contain exactly 36 items');
}

const finalReviewAreas = new Set(
  output.first_controlled_launch_final_review_items.map((item) => item.fixture_final_review_area),
);
for (const area of REQUIRED_FINAL_REVIEW_AREAS) {
  if (!finalReviewAreas.has(area)) fail(`required final review area missing: ${area}`);
}

for (const item of output.first_controlled_launch_final_review_items) {
  for (const field of REQUIRED_COMMON_FIELDS) {
    if (!(field in item)) {
      fail(
        `final review item ${item.fixture_final_review_id || 'unknown'} missing common field: ${field}`,
      );
    }
  }
  if (item.fixture_delivery_mode !== 'dry_run_only') {
    fail(`final review item ${item.fixture_final_review_id} fixture_delivery_mode is not dry_run_only`);
  }
  if (item.fixture_approval_status !== 'not_approved') {
    fail(`final review item ${item.fixture_final_review_id} fixture_approval_status is not not_approved`);
  }
  if (item.fixture_final_review_decision !== 'not_granted') {
    fail(`final review item ${item.fixture_final_review_id} fixture_final_review_decision is not not_granted`);
  }
  for (const flag of [
    'fixture_external_call_allowed',
    'fixture_live_activation_allowed',
    ...
    if (item[flag] !== false) {
      fail(`final review item ${item.fixture_final_review_id} ${flag} is not false`);
    }
  }
  if (!item.fixture_audit_event_id) {
    fail(`final review item ${item.fixture_final_review_id} missing fixture_audit_event_id`);
  }
}
passAssertion('final_review_items_have_common_fields');
passAssertion('final_review_items_remain_dry_run_only');
passAssertion('final_review_items_have_activation_flags_false');
passAssertion('final_review_decisions_are_not_granted');

if (!output.common_final_review_fields_summary.all_items_include_common_fields) {
  fail('common_final_review_fields_summary.all_items_include_common_fields must be true');
}
if (!output.first_controlled_launch_final_review_packet_dry_run_summary.all_items_remain_dry_run_only) {
  fail('first_controlled_launch_final_review_packet_dry_run_summary.all_items_remain_dry_run_only must be true');
}
if (!output.first_controlled_launch_final_review_packet_dry_run_summary.all_final_review_decisions_not_granted) {
  fail('first_controlled_launch_final_review_packet_dry_run_summary.all_final_review_decisions_not_granted must be true');
}
...
if (output.crm_handoff_export_final_review_summary.bidirectional_crm_sync_allowed !== false) {
  fail('crm handoff must remain not bidirectional sync');
}
if (output.lindy_bridge_final_review_summary.live_lindy_bridge_enabled !== false) {
  fail('live_lindy_bridge_enabled must remain false');
}
if (output.scheduler_dispatcher_final_review_summary.scheduler_enabled !== false) {
  fail('scheduler must remain disabled');
}
if (output.public_route_webhook_final_review_summary.public_route_enabled !== false) {
  fail('public route must remain disabled');
}
if (output.supabase_persistence_final_review_summary.production_write_allowed !== false) {
  fail('supabase production writes must remain blocked');
}
if (output.executive_final_review_summary.first_controlled_launch_allowed !== false) {
  fail('first_controlled_launch_allowed must remain false');
}
if (output.executive_final_review_summary.live_activation_allowed !== false) {
...
if (!output.readiness_lock_summary.readiness_lock_does_not_grant_approval) {
  fail('readiness_lock_summary.readiness_lock_does_not_grant_approval must be true');
}
if (!output.approval_request_summary.approval_request_packet_does_not_grant_approval) {
  fail('approval_request_summary.approval_request_packet_does_not_grant_approval must be true');
}
if (!output.execution_runbook_summary.execution_runbook_does_not_grant_approval_or_execute) {
  fail('execution_runbook_summary.execution_runbook_does_not_grant_approval_or_execute must be true');
}
if (!output.decision_ledger_summary.decision_ledger_does_not_grant_approval) {
  fail('decision_ledger_summary.decision_ledger_does_not_grant_approval must be true');
}
if (!output.audit_timeline_final_review_summary.all_items_have_audit_event_id) {
  fail('all final review items must have audit event id');
}
if (!output.owner_routing_final_review_summary.all_failures_routed_to_safe_owner) {
...
if (output.final_review_packet_toc_summary.total_areas !== 36) {
  fail('final_review_packet_toc_summary must have 36 areas');
}
...
if (output.final_reviewer_checklist_summary.checklist_complete !== false) {
  fail('final_reviewer_checklist_summary.checklist_complete must remain false');
}
...
for (const assertion of REQUIRED_ASSERTIONS) {
  if (!["first_controlled_launch_final_review_doc_present","fake_data_local_only_scope_present","explicit_non_approval_statement_present","first_controlled_launch_remains_blocked","sandbox_test_mode_activation_remains_blocked","live_activation_remains_blocked","relationship_to_decision_ledger_present","relationship_to_execution_runbook_present","relationship_to_approval_request_packet_present","relationship_to_readiness_lock_present","relationship_to_human_review_packet_present","final_review_packet_toc_present","executive_final_review_summary_present","evidence_chain_summary_present","readiness_lock_summary_present","approval_request_summary_present","execution_runbook_summary_present","decision_ledger_summary_present","approval_language_still_required_present","approval_signer_still_required_present","approval_timestamp_still_required_present","scope_requested_present","scope_excluded_present","channel_final_review_present","sms_final_review_present","email_final_review_present","call_vapi_final_review_present","google_calendar_final_review_present","csv_reporting_final_review_present","crm_handoff_export_final_review_present","lindy_bridge_final_review_present","scheduler_dispatcher_final_review_present","public_route_webhook_final_review_present","supabase_persistence_final_review_present","billing_payment_quote_boundary_blocked","credential_env_final_review_present","messaging_compliance_final_review_present","data_boundary_pii_final_review_present","audit_timeline_final_review_present","owner_routing_final_review_present","unresolved_blocker_register_present","rollback_confirmation_placeholder_present","post_approval_test_confirmation_placeholder_present","final_reviewer_checklist_present","allowed_next_actions_before_approval_present","forbidden_next_actions_before_approval_present","approval_not_granted","fast_lane_reference_present","runner_outputs_valid_json","final_review_items_have_common_fields","final_review_items_remain_dry_run_only","final_review_items_have_activation_flags_false","final_review_decisions_are_not_granted","no_live_sms_activation","no_twilio_activation","no_vapi_activation","no_resend_activation","no_google_calendar_activation","no_lindy_live_activation","no_scheduler_cron_dispatcher_activation","no_public_route_webhook_activation","no_crm_sync_activation","no_live_csv_delivery_activation","no_billing_payment_quote_invoice_estimate_activation","no_supabase_production_reads_writes","no_schema_migrations_auth_rls_security_changes","no_secret_env_credential_logging","demo_ready_with_live_automation_disabled_preserved","full_safe_readiness_lane_preserved","docs_and_context_wiring_present","dry_run_wrapper_present_and_safe","public_go_live_or_production_copy_not_changed_without_approval"].includes(assertion)) {
    fail(`unhandled assertion: ${assertion}`);
  }
}

console.log(
  `PASS: Native Workflow Fixture First Controlled Launch Final Review Packet Dry Run verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
