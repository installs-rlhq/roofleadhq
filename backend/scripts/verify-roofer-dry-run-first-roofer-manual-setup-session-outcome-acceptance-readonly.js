...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_OUTCOME_ACCEPTANCE.md');
const wrapper = read('scripts/accept-first-roofer-manual-setup-session-outcome-dry-run.sh');
...
[
  ...
  'internal only, founder/operator only, and session-outcome-acceptance-only',
  'MANUAL SETUP SESSION OUTCOME ACCEPTANCE PASS',
  'MANUAL SETUP SESSION OUTCOME ACCEPTANCE HOLD',
  'MANUAL SETUP SESSION OUTCOME ACCEPTANCE BLOCKED',
...
[
  ...
  'verify-roofer-dry-run-first-roofer-manual-setup-session-outcome-readonly.js',
  'verify-roofer-dry-run-first-roofer-manual-setup-session-outcome-acceptance-readonly.js',
  'scripts/check-production-gates.sh',
  'scripts/verify-safe-readiness.sh'
].forEach(s => { if (!wrapper.includes(s)) fail(`wrapper missing: ${s}`); });

if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-outcome-acceptance-readonly.js')) fail('aggregate missing session outcome acceptance verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_OUTCOME_ACCEPTANCE.md')) fail('index missing session outcome acceptance packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Outcome Acceptance Packet')) fail('first-paid context missing session outcome acceptance packet');
if (!roofer.includes('First Roofer Manual Setup Session Outcome Acceptance Packet')) fail('roofer context missing session outcome acceptance packet');

console.log('PASS: first-roofer manual setup session outcome acceptance verifier passed.');
