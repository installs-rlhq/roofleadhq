...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_ARCHIVE.md');
const wrapper = read('scripts/archive-first-roofer-manual-setup-session-dry-run.sh');
...
if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-archive-readonly.js')) fail('aggregate missing session archive verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_ARCHIVE.md')) fail('index missing session archive packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Archive Packet')) fail('first-paid context missing session archive packet');
if (!roofer.includes('First Roofer Manual Setup Session Archive Packet')) fail('roofer context missing session archive packet');

console.log('PASS: first-roofer manual setup session archive verifier passed.');
