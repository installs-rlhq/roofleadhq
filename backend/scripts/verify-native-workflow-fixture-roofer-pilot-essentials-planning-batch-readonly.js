...
const SOURCE_OF_TRUTH_COMMIT = '11e74d4';
...
const REQUIRED_ASSERTIONS = [
  'packet_doc_present',
  'sandbox_validation_doc_present',
  'setup_checklist_doc_present',
  'controlled_validation_doc_present',
  'management_summary_doc_present',
  ...
  'management_summary_fixture_present',
  ...
  'source_of_truth_commit_11e74d4_referenced',
  ...
  'recommended_scenario_counts_status_recommended_default_only_not_approved',
  'jason_review_packet_does_not_equal_approval',
  'release_candidate_summary_does_not_equal_approval',
  'management_summary_jason_review_does_not_equal_approval',
  ...
  'reviewed_management_summary_jason_review_status_completed',
  ...
  'recommended_sms_validation_count_5',
  'recommended_call_vapi_validation_count_3',
  'recommended_lead_intake_validation_count_5',
  'recommended_manual_review_escalation_count_4',
  'recommended_calendar_appointment_count_4',
  'recommended_reporting_admin_visibility_count_3',
  'recommended_audit_log_evidence_count_3',
  'recommended_stop_rollback_count_3',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_ROOFER_PILOT_ESSENTIALS_PLANNING_BATCH.md',
  'NATIVE_WORKFLOW_FIXTURE_SANDBOX_TEST_MODE_CHANNEL_VALIDATION_PLAN.md',
  'NATIVE_WORKFLOW_FIXTURE_REAL_ROOFER_PILOT_SETUP_CHECKLIST.md',
  'NATIVE_WORKFLOW_FIXTURE_CONTROLLED_REAL_ROOFER_VALIDATION_PLAN.md',
  'run-native-workflow-fixture-roofer-pilot-essentials-planning-batch-dry-run.sh',
  'verify-native-workflow-fixture-roofer-pilot-essentials-planning-batch-readonly.js',
  'roofer-pilot-essentials-planning-batch.json',
  'Native Workflow Fixture Roofer Pilot Essentials Planning Batch',
  'native workflow fixture roofer pilot essentials planning batch',
  'roofer pilot essentials planning batch',
];
...
const sandboxValidationDoc = read(sandboxValidationDocPath);
const setupChecklistDoc = read(setupChecklistDocPath);
const controlledValidationDoc = read(controlledValidationDocPath);
const managementSummaryDoc = read(managementSummaryDocPath);
...
const combinedDocs = `${packetDoc}\n${sandboxValidationDoc}\n${setupChecklistDoc}\n${controlledValidationDoc}\n${managementSummaryDoc}\n${releaseCandidateDoc}\n${decisionDraftDoc}\n${completenessEvidenceDoc}\n${localDemoEvidenceDoc}\n${readinessSummaryDoc}`;

passAssertion('packet_doc_present');
passAssertion('sandbox_validation_doc_present');
passAssertion('setup_checklist_doc_present');
passAssertion('controlled_validation_doc_present');
passAssertion('management_summary_doc_present');
...
passAssertion('source_of_truth_commit_11e74d4_referenced');
...
mustHave(packetDoc, 'Pilot planning does **not** equal approval', 'packet doc');
passAssertion('pilot_planning_does_not_equal_approval');
...
mustHave(packetDoc, 'RECOMMENDED_DEFAULT_ONLY_NOT_APPROVED', 'packet doc');
passAssertion('recommended_scenario_counts_status_recommended_default_only_not_approved');

mustHave(packetDoc, 'Jason review packet does **not** equal approval', 'packet doc');
passAssertion('jason_review_packet_does_not_equal_approval');
mustHave(packetDoc, 'Release candidate summary does **not** equal approval', 'packet doc');
passAssertion('release_candidate_summary_does_not_equal_approval');
mustHave(packetDoc, 'Management summary Jason review does **not** equal approval', 'packet doc');
passAssertion('management_summary_jason_review_does_not_equal_approval');
mustHave(packetDoc, 'Release candidate review does **not** equal approval', 'packet doc');
passAssertion('release_candidate_review_does_not_equal_approval');
mustHave(packetDoc, 'Evidence freeze does **not** equal approval', 'packet doc');
passAssertion('evidence_freeze_does_not_equal_approval');
mustHave(packetDoc, 'Evidence review does **not** equal approval', 'packet doc');
passAssertion('evidence_review_does_not_equal_approval');
mustHave(packetDoc, 'Decision draft does **not** equal approval', 'packet doc');
...
for (const [key, count] of Object.entries(RECOMMENDED_COUNTS)) {
  mustHave(packetDoc, `${key} | ${count}`, 'packet doc');
}
passAssertion('recommended_sms_validation_count_5');
passAssertion('recommended_call_vapi_validation_count_3');
passAssertion('recommended_lead_intake_validation_count_5');
passAssertion('recommended_manual_review_escalation_count_4');
passAssertion('recommended_calendar_appointment_count_4');
passAssertion('recommended_reporting_admin_visibility_count_3');
passAssertion('recommended_audit_log_evidence_count_3');
passAssertion('recommended_stop_rollback_count_3');
...
mustHave(packetDoc, 'does **not** run any activation command', 'packet doc');
...
if (!fs.existsSync(path.join(root, managementSummaryFixturePath))) {
  fail(`missing management summary fixture: ${managementSummaryFixturePath}`);
}
passAssertion('management_summary_fixture_present');
...
const managementSummaryFixture = readJson(managementSummaryFixturePath);
...
if (packetFixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('packet fixture source_of_truth_commit must be 11e74d4');
}
if (packetFixture.packet_status !== 'planning_only') {
  fail('packet fixture packet_status must be planning_only');
}
if (packetFixture.review_status !== 'pilot_planning_review_only') {
  fail('packet fixture review_status must be pilot_planning_review_only');
}
...
if (packetFixture.local_demo_evidence_freeze_release_candidate_review_status !== 'completed') {
  fail(
    'packet fixture local_demo_evidence_freeze_release_candidate_review_status must be completed',
  );
}
if (
  packetFixture.local_demo_release_candidate_management_summary_jason_review_status !== 'completed'
) {
  fail(
    'packet fixture local_demo_release_candidate_management_summary_jason_review_status must be completed',
  );
}
if (packetFixture.pilot_planning_status !== 'planning_only_not_approved') {
  fail('packet fixture pilot_planning_status must be planning_only_not_approved');
}
...
if (packetFixture.recommended_scenario_counts_are_not_approval !== true) {
  fail('packet fixture recommended_scenario_counts_are_not_approval must be true');
}
if (packetFixture.recommended_scenario_counts_status !== 'RECOMMENDED_DEFAULT_ONLY_NOT_APPROVED') {
  fail('packet fixture recommended_scenario_counts_status mismatch');
}
if (packetFixture.pilot_planning_does_not_equal_approval !== true) {
  fail('packet fixture pilot_planning_does_not_equal_approval must be true');
}
...
if (packetFixture.management_summary_jason_review_does_not_equal_approval !== true) {
  fail('packet fixture management_summary_jason_review_does_not_equal_approval must be true');
}
...
if (packetFixture.reviewed_management_summary_jason_review_status !== 'completed') {
  fail('packet fixture reviewed_management_summary_jason_review_status must be completed');
}
...
for (const [key, count] of Object.entries(RECOMMENDED_COUNTS)) {
  if (packetFixture.recommended_default_scenario_counts[key] !== count) {
    fail(`packet fixture recommended_default_scenario_counts.${key} must be ${count}`);
  }
}
passAssertion('reviewed_release_candidate_status_completed');
passAssertion('reviewed_management_summary_jason_review_status_completed');
...
if (!Array.isArray(packetFixture.approved_channels) || packetFixture.approved_channels.length !== 0) {
...
if (managementSummaryFixture.packet_status !== 'review_only') {
  fail('management summary fixture packet_status must be review_only');
}
if (managementSummaryFixture.review_status !== 'management_summary_jason_review_only') {
  fail('management summary fixture review_status must be management_summary_jason_review_only');
}
if (managementSummaryFixture.approval_status !== 'not_granted') {
  fail('management summary fixture approval_status must be not_granted');
}

if (
  !Array.isArray(packetFixture.evidence_chain_commits) ||
  packetFixture.evidence_chain_commits.length !== 19
) {
  fail('packet fixture evidence_chain_commits must contain 19 commits');
}
for (const commit of EVIDENCE_CHAIN_COMMITS) {
...
mustHave(wrapper, 'Pilot planning does not equal approval', 'wrapper mode');
...
mustHave(packetDoc, sandboxValidationDocPath, 'packet doc');
mustHave(packetDoc, setupChecklistDocPath, 'packet doc');
mustHave(packetDoc, controlledValidationDocPath, 'packet doc');
mustHave(packetDoc, managementSummaryDocPath, 'packet doc');
mustHave(packetDoc, managementSummaryFixturePath, 'packet doc');
...
if (REQUIRED_ASSERTIONS.length !== 118) {
  fail(`REQUIRED_ASSERTIONS must contain 118 items (found ${REQUIRED_ASSERTIONS.length})`);
}

console.log(
  `PASS: Native Workflow Fixture Roofer Pilot Essentials Planning Batch verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
