...
const SOURCE_OF_TRUTH_COMMIT = '3b7719b';
...
const FRESH_RUNNER_EXECUTION_CAPTURE_CHECKS = [
  'Build 119 fresh runner-execution decision after after-guard wiring correction packet referenced upstream and closed at 3b7719b',
  'Build 118 runner execution path after-guard wiring correction and Build 117 blocked command evidence referenced',
  'runner command path exists executable and corrected fail-closed pending fresh execution pre-run guard after after-guard wiring correction approval capture',
  ...
  'future_command_status blocked until fresh execution pre-run guard passes after after-guard wiring correction approval capture',
];

const REQUIRED_APPROVAL_LANGUAGE = [
  'I, Jason Lohse, approve the fresh runner-execution approval capture after after-guard wiring correction for RoofLeadHQ.',
  'Scope approved:',
  'fresh_run_actual_external_sandbox_30_scenario_validation_once_only_after_after_guard_wiring_correction',
  ...
  '30',
  'I approve only a scoped sandbox/test-mode runner-execution approval capture packet after the Build 118 after-guard wiring correction and Build 119 fresh decision/template packet.',
  ...
  'I understand Build 117 consumed the prior post-Build-116 attempt and no prior approval/guard chain is reusable.',
  'Signed: Jason Lohse',
  'Date/time: 06/20/2026 12:06pm MST',
];

const REQUIRED_ASSERTIONS = [
  'packet_doc_present',
  'source_of_truth_commit_3b7719b_referenced',
  ...
  'future_command_status_blocked_until_fresh_execution_pre_run_guard_passes_after_after_guard_wiring_correction_approval_capture',
  ...
  'fresh_runner_execution_approval_capture_after_after_guard_wiring_correction_dry_run_wrapper_present_and_safe',
  'wrapper_does_not_invoke_runner',
  'verifier_file_present',
];

const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_CAPTURE_FRESH_RUNNER_EXECUTION_APPROVAL_AFTER_AFTER_GUARD_WIRING_CORRECTION.md',
  'run-native-workflow-fixture-capture-fresh-runner-execution-approval-after-after-guard-wiring-correction-dry-run.sh',
  'verify-native-workflow-fixture-capture-fresh-runner-execution-approval-after-after-guard-wiring-correction-readonly.js',
  'capture-fresh-runner-execution-approval-after-after-guard-wiring-correction.json',
  'Native Workflow Fixture Capture Fresh Runner-Execution Approval After After-Guard Wiring Correction',
  'native workflow fixture capture fresh runner execution approval after after-guard wiring correction',
  'capture fresh runner execution approval after after-guard wiring correction',
];
...
passAssertion('source_of_truth_commit_3b7719b_referenced');
...
mustHave(packetDoc, 'fresh_runner_execution_scope_approved | fresh_run_actual_external_sandbox_30_scenario_validation_once_only_after_after_guard_wiring_correction', 'packet doc');
...
passAssertion('future_command_status_blocked_until_fresh_execution_pre_run_guard_passes_after_after_guard_wiring_correction_approval_capture');
...
mustHave(
  packetDoc,
  'This packet captures fresh runner-execution approval after after-guard wiring correction **only**.',
  'packet doc',
);
...
mustHave(packetDoc, 'fresh execution pre-run guard pass after after-guard wiring correction approval capture**, not execution', 'packet doc');
...
const upstreamPathCorrectionFixture = readJson(upstreamAfterGuardWiringCorrectionFixturePath);
const upstreamBlockedEvidenceFixture = readJson(upstreamPostBuild116BlockedEvidenceFixturePath);
const upstreamFreshPreRunGuardFixture = readJson(upstreamFreshPreRunGuardAfterPathCorrectionFixturePath);
const upstreamPriorCaptureFixture = readJson(upstreamCaptureAfterPathCorrectionFixturePath);
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be 3b7719b');
}
if (
...
if (fixture.prior_post_build_116_command_attempt_consumption_status !== 'consumed_by_blocked_fail_closed_result_after_build_116_guard') {
...
if (
  ...
  fail(
    'upstream fresh decision after after-guard wiring correction fixture fresh_runner_execution_decision_template_status must be created_review_only',
  );
}
if (
  upstreamFreshDecisionAfterAfterGuardWiringCorrectionFixture.fresh_runner_execution_approval_capture_status !==
  'not_captured'
) {
  fail(
    'upstream fresh decision after after-guard wiring correction fixture fresh_runner_execution_approval_capture_status must be not_captured (prior to this capture packet)',
  );
}
...
mustHave(wrapper, 'local fresh runner-execution approval capture after after-guard wiring correction only', 'wrapper mode');
...
mustHave(
  wrapper,
  'future_command_status is blocked_until_fresh_execution_pre_run_guard_passes_after_after_guard_wiring_correction_approval_capture',
  'wrapper mode',
);
...
if (!isExecutable(wrapperPath)) {
  fail(
    `fresh runner-execution approval capture after after-guard wiring correction dry-run wrapper is not executable: ${wrapperPath}`,
  );
}
passAssertion('fresh_runner_execution_approval_capture_after_after_guard_wiring_correction_dry_run_wrapper_present_and_safe');
...
console.log(
  `PASS: Native Workflow Fixture Capture Fresh Runner-Execution Approval After After-Guard Wiring Correction verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
