...
const requiredDocStrings = [
  'First Roofer Manual Setup Session Reopen Guard Final Lock Acceptance Reopen Guard Final Lock Acceptance Preservation Snapshot',
...
const requiredWrapperStrings = [
  ...
  'verify-roofer-dry-run-first-roofer-manual-setup-session-reopen-guard-final-lock-acceptance-preservation-snapshot-readonly.js',
...
const requiredIntegrationChecks = {
  ...
  'docs/FIRST_PAID_LAUNCH_VERIFIER_INDEX.md': [
    'verify-roofer-dry-run-first-roofer-manual-setup-session-reopen-guard-final-lock-acceptance-preservation-snapshot-readonly.js',
    'ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_REOPEN_GUARD_FINAL_LOCK_ACCEPTANCE_PRESERVATION_SNAPSHOT.md',
    'snapshot-first-roofer-manual-setup-session-reopen-guard-final-lock-acceptance-preservation-dry-run.sh',
  ],
...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_REOPEN_GUARD_FINAL_LOCK_ACCEPTANCE_PRESERVATION_SNAPSHOT.md');
const wrapper = read('scripts/snapshot-first-roofer-manual-setup-session-reopen-guard-final-lock-acceptance-preservation-dry-run.sh');
...
for (const [file, requiredStrings] of Object.entries(requiredIntegrationChecks)) {
  ...
  for (const value of requiredStrings) {
    assert(content.includes(value), `${file} missing reopen guard final lock acceptance preservation snapshot integration text: ${value}`);
  }
}

console.log('PASS: First roofer manual setup session reopen guard final lock acceptance preservation snapshot verifier passed.');
