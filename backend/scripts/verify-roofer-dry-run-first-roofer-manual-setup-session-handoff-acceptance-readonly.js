...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_HANDOFF_ACCEPTANCE.md');
const wrapper = read('scripts/accept-first-roofer-manual-setup-session-handoff-dry-run.sh');
...
[
  ...
  'internal only, founder/operator only, and session-handoff-acceptance-only',
  'MANUAL SETUP SESSION HANDOFF ACCEPTANCE PASS',
  'MANUAL SETUP SESSION HANDOFF ACCEPTANCE HOLD',
  'MANUAL SETUP SESSION HANDOFF ACCEPTANCE BLOCKED',
...
[
  ...
  'verify-roofer-dry-run-first-roofer-manual-setup-session-handoff-readonly.js',
...
if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-handoff-acceptance-readonly.js')) fail('aggregate missing session handoff acceptance verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_HANDOFF_ACCEPTANCE.md')) fail('index missing session handoff acceptance packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Handoff Acceptance Packet')) fail('first-paid context missing session handoff acceptance packet');
if (!roofer.includes('First Roofer Manual Setup Session Handoff Acceptance Packet')) fail('roofer context missing session handoff acceptance packet');

console.log('PASS: first-roofer manual setup session handoff acceptance verifier passed.');
