...
const requiredDocStrings = [
  ...
  'operator handoff freeze has passed',
  'prevents accidental mutation of the final accepted session record',
...
const requiredIntegrationChecks = {
  ...
  'docs/FIRST_PAID_LAUNCH_VERIFIER_INDEX.md': [
    'verify-roofer-dry-run-first-roofer-manual-setup-session-reopen-guard-readonly.js',
    'ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_REOPEN_GUARD.md',
    'check-first-roofer-manual-setup-session-reopen-guard-dry-run.sh',
  ],
...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_REOPEN_GUARD.md');
const wrapper = read('scripts/check-first-roofer-manual-setup-session-reopen-guard-dry-run.sh');

for (const value of requiredDocStrings) {
  assert(doc.includes(value), `Reopen guard doc missing required text: ${value}`);
}

for (const value of requiredWrapperStrings) {
  assert(wrapper.includes(value), `Reopen guard wrapper missing required command: ${value}`);
}

for (const value of forbiddenWrapperStrings) {
  assert(!wrapper.includes(value), `Reopen guard wrapper contains forbidden live/prod pattern: ${value}`);
}

for (const [file, requiredStrings] of Object.entries(requiredIntegrationChecks)) {
  const content = read(file);
  for (const value of requiredStrings) {
    assert(content.includes(value), `${file} missing reopen guard integration text: ${value}`);
  }
}

console.log('PASS: First roofer manual setup session reopen guard verifier passed.');
