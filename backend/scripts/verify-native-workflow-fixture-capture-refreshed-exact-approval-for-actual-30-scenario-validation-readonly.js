...
const SOURCE_OF_TRUTH_COMMIT = 'ae61d53';
...
const REFRESHED_CAPTURE_CHECKS = [
  'prior one-time approval consumed by local wrapper execution',
  'actual 30-scenario external validation remains 0 captured',
  'refreshed exact approval template packet referenced upstream',
  'refreshed signed approval captured and signed',
  'all 19 refreshed exact values accepted and approved',
  'no command execution by this packet',
  'no external calls credentials production data or contact authorized by this packet',
  'live activation and real contact blocks remain not_granted',
  'controlled real roofer setup remains blocked',
  'future_command_status blocked until refreshed pre-run guard passes',
];
...
const REQUIRED_ASSERTIONS = [
  ...
  'upstream_refreshed_template_fixture_present',
  ...
  'source_of_truth_commit_ae61d53_referenced',
  ...
  'refreshed_exact_approval_template_packet_referenced',
  ...
  'refreshed_signed_approval_text_present',
  ...
  'future_command_status_blocked_until_refreshed_pre_run_guard_passes',
  ...
  'packet_captures_refreshed_signed_approval_evidence_only',
  'packet_does_not_execute_approved_command',
  'packet_does_not_pass_refreshed_pre_run_guard',
  ...
  'next_step_refreshed_pre_run_guard_required',
  'demo_ready_with_live_automation_disabled_preserved',
  'full_safe_readiness_lane_preserved',
  'docs_and_context_wiring_present',
  'refreshed_capture_dry_run_wrapper_present_and_safe',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_CAPTURE_REFRESHED_EXACT_APPROVAL_FOR_ACTUAL_30_SCENARIO_VALIDATION.md',
  'NATIVE_WORKFLOW_FIXTURE_CAPTURE_REFRESHED_EXACT_APPROVAL_FOR_ACTUAL_30_SCENARIO_VALIDATION_NO_GO_REVIEW.md',
  'run-native-workflow-fixture-capture-refreshed-exact-approval-for-actual-30-scenario-validation-dry-run.sh',
  'verify-native-workflow-fixture-capture-refreshed-exact-approval-for-actual-30-scenario-validation-readonly.js',
  'capture-refreshed-exact-approval-for-actual-30-scenario-validation.json',
  'Native Workflow Fixture Capture Refreshed Exact Approval for Actual 30-Scenario Validation',
  'native workflow fixture capture refreshed exact approval for actual 30 scenario validation',
  'capture refreshed exact approval for actual 30 scenario validation',
];
...
mustHave(
  packetDoc,
  'fake-data/local-only/read-only/dry-run-only/review-only/refreshed-approval-capture-only/non-executing',
  'packet doc',
);
mustHave(
  noGoReviewDoc,
...
passAssertion('source_of_truth_commit_ae61d53_referenced');
...
mustHave(packetDoc, refreshedTemplateDocPath, 'packet doc');
mustHave(packetDoc, refreshedTemplateFixturePath, 'packet doc');
mustHave(packetDoc, 'refreshed-exact-approval-for-actual-30-scenario-validation', 'packet doc');
passAssertion('refreshed_exact_approval_template_packet_referenced');
...
for (const line of REQUIRED_APPROVAL_LANGUAGE) {
  mustHave(packetDoc, line, 'packet doc refreshed signed approval');
}
passAssertion('refreshed_signed_approval_text_present');
...
passAssertion('future_command_status_blocked_until_refreshed_pre_run_guard_passes');
...
mustHave(packetDoc, 'This packet captures refreshed signed approval evidence **only**', 'packet doc');
passAssertion('packet_captures_refreshed_signed_approval_evidence_only');
...
mustHave(packetDoc, 'This packet does **not** pass the refreshed pre-run guard by itself', 'packet doc');
passAssertion('packet_does_not_pass_refreshed_pre_run_guard');
...
mustHave(packetDoc, 'separate refreshed pre-run guard pass', 'packet doc');
passAssertion('next_step_refreshed_pre_run_guard_required');
...
for (const check of REFRESHED_CAPTURE_CHECKS) {
  mustHave(packetDoc, check, 'packet doc refreshed capture checks');
  mustHave(packetDoc, `| ${check} | passed |`, 'packet doc refreshed capture check status');
}
...
passAssertion('upstream_refreshed_template_fixture_present');
...
const upstreamTemplateFixture = readJson(refreshedTemplateFixturePath);
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be ae61d53');
}
...
if (fixture.future_command_status !== 'blocked_until_refreshed_pre_run_guard_passes') {
...
if (!fixture.captured_refreshed_jason_signed_approval_statement) {
  fail('fixture captured_refreshed_jason_signed_approval_statement missing');
}
if (fixture.captured_refreshed_jason_signed_approval_statement.captured !== true) {
  fail('fixture captured_refreshed_jason_signed_approval_statement.captured must be true');
}
if (fixture.captured_refreshed_jason_signed_approval_statement.signed !== true) {
  fail('fixture captured_refreshed_jason_signed_approval_statement.signed must be true');
}
if (fixture.captured_refreshed_jason_signed_approval_statement.activation_granted !== false) {
  fail('fixture captured_refreshed_jason_signed_approval_statement.activation_granted must be false');
}

if (
  !Array.isArray(fixture.refreshed_approval_capture_checks) ||
  fixture.refreshed_approval_capture_checks.length !== 10
) {
  fail('fixture refreshed_approval_capture_checks must contain 10 checks');
}
for (let i = 0; i < REFRESHED_CAPTURE_CHECKS.length; i += 1) {
  const check = fixture.refreshed_approval_capture_checks[i];
  if (check.check !== i + 1) {
    fail(`fixture refreshed_approval_capture_checks[${i}].check must be ${i + 1}`);
  }
  if (check.name !== REFRESHED_CAPTURE_CHECKS[i]) {
    fail(`fixture refreshed_approval_capture_checks[${i}].name mismatch`);
  }
  if (check.status !== 'passed') {
    fail(`fixture refreshed_approval_capture_checks[${i}].status must be passed`);
  }
}

if (
  !Array.isArray(fixture.evidence_chain_commits) ||
  fixture.evidence_chain_commits.length !== 42
) {
  fail('fixture evidence_chain_commits must contain 42 commits');
}
for (const commit of EVIDENCE_CHAIN_COMMITS) {
...
if (upstreamTemplateFixture.source_of_truth_commit !== '6411949') {
  fail('upstream refreshed template fixture source_of_truth_commit must be 6411949');
}
...
for (const pattern of UNSAFE_PATTERNS) {
  ...
  if (pattern.test(wrapper)) fail(`unsafe pattern ${pattern} found in refreshed capture wrapper`);
}
...
mustHave(wrapper, 'local refreshed approval-capture only', 'wrapper mode');
...
mustHave(wrapper, 'refreshed pre-run guard pass', 'wrapper mode');
...
mustHave(
  wrapper,
  'future_command_status is blocked_until_refreshed_pre_run_guard_passes',
  'wrapper mode',
);
...
if (!isExecutable(wrapperPath)) {
  fail(`refreshed capture wrapper is not executable: ${wrapperPath}`);
}
passAssertion('refreshed_capture_dry_run_wrapper_present_and_safe');
...
console.log(
  `PASS: Native Workflow Fixture Capture Refreshed Exact Approval for Actual 30-Scenario Validation verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
