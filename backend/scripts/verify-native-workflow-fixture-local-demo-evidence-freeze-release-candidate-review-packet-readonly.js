...
const SOURCE_OF_TRUTH_COMMIT = 'ef79784';
...
const RECOMMENDED_NEXT_STEP =
  'JASON_REVIEW_LOCAL_DEMO_EVIDENCE_FREEZE_RELEASE_CANDIDATE_HOLD_UNTIL_EXACT_VALUES_AND_SEPARATE_SANDBOX_TEST_MODE_APPROVAL';
...
const REQUIRED_ASSERTIONS = [
  ...
  'source_of_truth_commit_ef79784_referenced',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_LOCAL_DEMO_EVIDENCE_FREEZE_RELEASE_CANDIDATE_REVIEW_PACKET.md',
  'run-native-workflow-fixture-local-demo-evidence-freeze-release-candidate-review-packet-dry-run.sh',
  'verify-native-workflow-fixture-local-demo-evidence-freeze-release-candidate-review-packet-readonly.js',
  'local-demo-evidence-freeze-release-candidate-review-packet.json',
  'Native Workflow Fixture Local Demo Evidence Freeze / Release Candidate Review Packet',
  'native workflow fixture local demo evidence freeze release candidate review packet',
  'local demo evidence freeze release candidate review packet',
];
...
passAssertion('source_of_truth_commit_ef79784_referenced');
...
if (packetFixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('packet fixture source_of_truth_commit must be ef79784');
}
if (packetFixture.packet_status !== 'review_only') {
  fail('packet fixture packet_status must be review_only');
}
if (packetFixture.review_status !== 'release_candidate_review_only') {
  fail('packet fixture review_status must be release_candidate_review_only');
}
...
if (
  ...
  packetFixture.evidence_chain_commits.length !== 17
) {
  fail('packet fixture evidence_chain_commits must contain 17 commits');
}
for (const commit of EVIDENCE_CHAIN_COMMITS) {
...
mustHave(wrapper, 'release candidate review packet', 'wrapper mode');
mustHave(wrapper, 'Release candidate review does not equal approval', 'wrapper mode');
mustHave(wrapper, 'Evidence freeze does not equal approval', 'wrapper mode');
...
if (REQUIRED_ASSERTIONS.length !== 88) {
  fail(`REQUIRED_ASSERTIONS must contain 88 items (found ${REQUIRED_ASSERTIONS.length})`);
}

console.log(
  `PASS: Native Workflow Fixture Local Demo Evidence Freeze / Release Candidate Review Packet verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
