...
const requiredIntegrationChecks = {
  ...
  'docs/FIRST_PAID_LAUNCH_VERIFIER_INDEX.md': [
    'verify-roofer-dry-run-first-roofer-manual-setup-session-preservation-snapshot-readonly.js',
    'ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_PRESERVATION_SNAPSHOT.md',
    'snapshot-first-roofer-manual-setup-session-preservation-dry-run.sh',
  ],
...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_PRESERVATION_SNAPSHOT.md');
const wrapper = read('scripts/snapshot-first-roofer-manual-setup-session-preservation-dry-run.sh');
...
for (const [file, requiredStrings] of Object.entries(requiredIntegrationChecks)) {
  ...
  for (const value of requiredStrings) {
    assert(content.includes(value), `${file} missing preservation snapshot integration text: ${value}`);
  }
}

console.log('PASS: First roofer manual setup session preservation snapshot verifier passed.');
