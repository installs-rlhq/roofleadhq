...
const SOURCE_OF_TRUTH_COMMIT = '2ea4c2e';
...
const FRESH_RUNNER_EXECUTION_CAPTURE_CHECKS = [
  'Build 114 fresh runner-execution decision after path correction packet referenced upstream and closed at 2ea4c2e',
  'Build 113 runner execution path correction and Build 112 blocked command evidence referenced',
  'runner command path exists executable and corrected fail-closed pending fresh execution pre-run guard after path correction approval capture',
  ...
  'future_command_status blocked until fresh execution pre-run guard passes after path correction approval capture',
];

const REQUIRED_APPROVAL_LANGUAGE = [
  'I, Jason Lohse, approve the fresh runner-execution approval capture after path correction for RoofLeadHQ.',
  'Scope approved:',
  'fresh_run_actual_external_sandbox_30_scenario_validation_once_only_after_path_correction',
  ...
  '30',
  'I approve only a scoped sandbox/test-mode runner-execution approval capture packet after the Build 113 path correction and Build 114 fresh decision/template packet.',
  ...
  'I understand Build 112 consumed the prior fresh attempt and no prior approval is reusable.',
  'Signed: Jason Lohse',
  'Date/time: 06/20/2026 11:26am MST',
];

const REQUIRED_ASSERTIONS = [
  'packet_doc_present',
  'source_of_truth_commit_2ea4c2e_referenced',
  ...
  'runner_command_path_status_corrected_fail_closed_direct_invocation_always_blocked_pending_fresh_decision_after_execution_path_correction',
  ...
  'future_command_status_blocked_until_fresh_execution_pre_run_guard_passes_after_path_correction_approval_capture',
  ...
  'fresh_runner_execution_approval_capture_after_path_correction_dry_run_wrapper_present_and_safe',
  'wrapper_does_not_invoke_runner',
  'verifier_file_present',
];

const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_CAPTURE_FRESH_RUNNER_EXECUTION_APPROVAL_AFTER_PATH_CORRECTION.md',
  'run-native-workflow-fixture-capture-fresh-runner-execution-approval-after-path-correction-dry-run.sh',
  'verify-native-workflow-fixture-capture-fresh-runner-execution-approval-after-path-correction-readonly.js',
  'capture-fresh-runner-execution-approval-after-path-correction.json',
  'Native Workflow Fixture Capture Fresh Runner-Execution Approval After Path Correction',
  'native workflow fixture capture fresh runner execution approval after path correction',
  'capture fresh runner execution approval after path correction',
];
...
passAssertion('source_of_truth_commit_2ea4c2e_referenced');
...
passAssertion('runner_command_path_status_corrected_fail_closed_direct_invocation_always_blocked_pending_fresh_decision_after_execution_path_correction');
...
passAssertion('future_command_status_blocked_until_fresh_execution_pre_run_guard_passes_after_path_correction_approval_capture');
...
mustHave(
  packetDoc,
  'This packet captures fresh runner-execution approval after path correction **only**.',
  'packet doc',
);
...
mustHave(packetDoc, 'fresh execution pre-run guard pass after path correction approval capture**, not execution', 'packet doc');
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be 2ea4c2e');
}
if (
...
if (
  upstreamFreshDecisionAfterPathCorrectionFixture.fresh_runner_execution_approval_capture_status !==
  'not_captured'
) {
  fail(
    'upstream fresh decision after path correction fixture fresh_runner_execution_approval_capture_status must be not_captured (prior to this capture packet)',
  );
}
...
mustHave(wrapper, 'local fresh runner-execution approval capture after path correction only', 'wrapper mode');
...
mustHave(
  wrapper,
  'future_command_status is blocked_until_fresh_execution_pre_run_guard_passes_after_path_correction_approval_capture',
  'wrapper mode',
);
...
if (!isExecutable(wrapperPath)) {
  fail(
    `fresh runner-execution approval capture after path correction dry-run wrapper is not executable: ${wrapperPath}`,
  );
}
passAssertion('fresh_runner_execution_approval_capture_after_path_correction_dry_run_wrapper_present_and_safe');
...
console.log(
  `PASS: Native Workflow Fixture Capture Fresh Runner-Execution Approval After Path Correction verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
