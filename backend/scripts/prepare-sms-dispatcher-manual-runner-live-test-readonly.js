...
const MANUAL_RUNNER_TARGET = 'sms_dispatcher_manual_test_runner';
...
console.log('=== RoofLeadHQ Manual Dispatcher Runner Live Test Prep Read-Only ===');
...
function suggestedRunId(now = new Date()) {
  return `manual-runner-live-prep-${now.toISOString().replace(/[:.]/g, '-').replace('Z', 'z')}`;
}

function buildProviderTestId(runId) {
  return `sms-dispatcher-manual-test-${runId}`;
}
...
function followUpSelectColumns(includeCreatedAt) {
  return `
      ...
      roofers(id, business_name, status, sms_confirmation_enabled, timezone)
    `;
}
...
function runStaticSafetyChecks() {
  ...
  const forbidden = [
    ...
    { pattern: /runProductionSmsDispatcher\s*\(/i, label: 'production dispatcher activation' }
  ];
  ...
  pass('static safety checks found no writes, Twilio, SMS send, routes, cron, or dispatcher activation');
}
...
async function findDuplicateByBody(supabase, candidate) {
  if (!candidate.message_body) return { data: [], error: null };
...
async function main() {
  ...
  const rooferId = getArgValue('--roofer-id') || process.env.SMS_MANUAL_TEST_ROOFER_ID || TEST_ROOFER_ID;
  ...
  const runId = getArgValue('--run-id') || process.env.SMS_MANUAL_TEST_RUN_ID || suggestedRunId();
  ...
  console.log('=== SELECTED MANUAL RUNNER LIVE TEST CANDIDATE ===');
  ...
  console.log('=== FUTURE MANUAL RUNNER LIVE TEST ENV ===');
  console.log('export SMS_MANUAL_TEST_USE_LIVE_SUPABASE=true');
  console.log('export SMS_MANUAL_TEST_ONLY=true');
  console.log('export SMS_MANUAL_TEST_RUNNER=true');
  console.log(`export SMS_MANUAL_TEST_TARGET=${MANUAL_RUNNER_TARGET}`);
  console.log(`export SMS_MANUAL_TEST_ROOFER_ID=${rooferId}`);
  console.log(`export SMS_MANUAL_TEST_RUN_ID=${runId}`);
  console.log(`export SMS_MANUAL_TEST_MAX_BATCH_SIZE=${MAX_BATCH_SIZE}`);
  ...
  console.log('=== FUTURE MANUAL RUNNER LIVE TEST COMMAND - DO NOT RUN WITHOUT APPROVAL ===');
  console.log('node backend/scripts/run-sms-dispatcher-manual-test-only.js \\');
  console.log('  --test-only \\');
  console.log('  --allow-live-supabase-manual-test \\');
  console.log(`  --approved-roofer-id ${rooferId} \\`);
  console.log(`  --max-batch-size ${MAX_BATCH_SIZE}`);
  console.log('PASS: read-only manual runner live test prep completed.');
}
