...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_OUTCOME.md');
const wrapper = read('scripts/record-first-roofer-manual-setup-session-outcome-dry-run.sh');
...
[
  ...
  'internal only, founder/operator only, and session-outcome-only',
  'MANUAL SETUP SESSION OUTCOME PASS',
  'MANUAL SETUP SESSION OUTCOME HOLD',
  'MANUAL SETUP SESSION OUTCOME BLOCKED',
...
if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-outcome-readonly.js')) fail('aggregate missing session outcome verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_OUTCOME.md')) fail('index missing session outcome packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Outcome Packet')) fail('first-paid context missing session outcome packet');
if (!roofer.includes('First Roofer Manual Setup Session Outcome Packet')) fail('roofer context missing session outcome packet');

console.log('PASS: first-roofer manual setup session outcome verifier passed.');
