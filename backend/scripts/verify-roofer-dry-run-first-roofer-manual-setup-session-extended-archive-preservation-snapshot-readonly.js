...
const docPath = 'docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_EXTENDED_ARCHIVE_PRESERVATION_SNAPSHOT.md';
const wrapperPath = 'scripts/snapshot-first-roofer-manual-setup-session-extended-archive-preservation-dry-run.sh';
const verifierPath = 'backend/scripts/verify-roofer-dry-run-first-roofer-manual-setup-session-extended-archive-preservation-snapshot-readonly.js';
...
[
  ...
  'internal only, founder/operator only, and extended archive-preservation-snapshot-only',
...
if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-extended-archive-preservation-snapshot-readonly.js')) fail('aggregate missing extended archive preservation snapshot verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_EXTENDED_ARCHIVE_PRESERVATION_SNAPSHOT.md')) fail('index missing extended archive preservation snapshot packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Extended Archive Preservation Snapshot Packet')) fail('first-paid context missing extended archive preservation snapshot packet');
if (!roofer.includes('First Roofer Manual Setup Session Extended Archive Preservation Snapshot Packet')) fail('roofer context missing extended archive preservation snapshot packet');

console.log('PASS: first-roofer manual setup session extended archive preservation snapshot verifier passed.');
