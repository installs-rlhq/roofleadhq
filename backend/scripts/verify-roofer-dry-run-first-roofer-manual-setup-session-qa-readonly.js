...
for (const f of requiredFiles) read(f);

const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_QA.md');
const wrapper = read('scripts/qa-first-roofer-manual-setup-session-dry-run.sh');
...
[
  ...
  'internal only, founder/operator only, and session-QA-only',
  'MANUAL SETUP SESSION QA PASS',
  'MANUAL SETUP SESSION QA HOLD',
  'MANUAL SETUP SESSION QA BLOCKED',
...
].forEach(s => { if (!doc.includes(s)) fail(`session QA doc missing: ${s}`); });
...
[
  ...
  'verify-roofer-dry-run-first-roofer-manual-setup-session-notes-readonly.js',
...
if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-qa-readonly.js')) fail('aggregate missing session QA verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_QA.md')) fail('index missing session QA packet');
if (!firstPaid.includes('First Roofer Manual Setup Session QA Packet')) fail('first-paid context missing session QA packet');
if (!roofer.includes('First Roofer Manual Setup Session QA Packet')) fail('roofer context missing session QA packet');

console.log('PASS: first-roofer manual setup session QA verifier passed.');
