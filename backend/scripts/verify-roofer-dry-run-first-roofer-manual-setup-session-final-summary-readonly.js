...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_FINAL_SUMMARY.md');
const wrapper = read('scripts/summarize-first-roofer-manual-setup-session-dry-run.sh');
...
[
  ...
  'internal only, founder/operator only, and session-final-summary-only',
  'MANUAL SETUP SESSION FINAL SUMMARY PASS',
  'MANUAL SETUP SESSION FINAL SUMMARY HOLD',
  'MANUAL SETUP SESSION FINAL SUMMARY BLOCKED',
...
if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-final-summary-readonly.js')) fail('aggregate missing session final summary verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_FINAL_SUMMARY.md')) fail('index missing session final summary packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Final Summary Packet')) fail('first-paid context missing session final summary packet');
if (!roofer.includes('First Roofer Manual Setup Session Final Summary Packet')) fail('roofer context missing session final summary packet');

console.log('PASS: first-roofer manual setup session final summary verifier passed.');
