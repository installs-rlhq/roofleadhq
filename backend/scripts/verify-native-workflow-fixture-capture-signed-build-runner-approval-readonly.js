...
const SOURCE_OF_TRUTH_COMMIT = '07421c8';
...
const EXACT_VALUE_FIELDS = [
  ...
  'build_runner_jason_signed_approval_status',
  'build_runner_exact_values_required_count',
  'build_runner_exact_values_accepted_count',
  'build_runner_exact_values_approved_count',
  'runner_execution_approval_status',
  'external_calls_approval_status',
  ...
  'billing_payment_automation_approval_status',
...
const APPROVED_EXACT_VALUES = {
  ...
  build_runner_approval_capture_status: 'approved_for_capture',
  ...
  command_execution_status: 'not_run_by_this_approval',
};

const BUILD_RUNNER_CAPTURE_CHECKS = [
  'Build 100 build-runner exact approval template packet referenced upstream',
  ...
  'build runner signed approval captured and signed',
  'all 19 build runner exact values accepted and approved',
  'no runner build by this packet',
  ...
  'future_command_status blocked until build runner pre-run guard passes',
];
...
const REQUIRED_ASSERTIONS = [
  ...
  'source_of_truth_commit_07421c8_referenced',
  ...
  'all_19_build_runner_exact_values_present_and_approved',
  ...
  'future_command_status_blocked_until_build_runner_pre_run_guard_passes',
  ...
  'packet_captures_signed_build_runner_approval_evidence_only',
  'packet_does_not_build_runner',
  'packet_does_not_run_runner',
  'packet_does_not_pass_build_runner_pre_run_guard',
  ...
  'next_step_build_runner_pre_run_guard_or_scaffolding_build_packet',
  'build_runner_signed_approval_capture_checks_all_10_checks_present',
  'build_runner_signed_approval_capture_checks_all_10_checks_passed',
  'demo_ready_with_live_automation_disabled_preserved',
  'full_safe_readiness_lane_preserved',
  'docs_and_context_wiring_present',
  'signed_build_runner_capture_dry_run_wrapper_present_and_safe',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_CAPTURE_SIGNED_BUILD_RUNNER_APPROVAL.md',
  'NATIVE_WORKFLOW_FIXTURE_CAPTURE_SIGNED_BUILD_RUNNER_APPROVAL_NO_GO_REVIEW.md',
  'run-native-workflow-fixture-capture-signed-build-runner-approval-dry-run.sh',
  'verify-native-workflow-fixture-capture-signed-build-runner-approval-readonly.js',
  'capture-signed-build-runner-approval.json',
  'Native Workflow Fixture Capture Signed Build-Runner Approval',
  'native workflow fixture capture signed build runner approval',
  'capture signed build runner approval',
];
...
passAssertion('source_of_truth_commit_07421c8_referenced');
...
for (const field of EXACT_VALUE_FIELDS) {
  ...
  mustHave(packetDoc, `| ${field} | ${APPROVED_EXACT_VALUES[field]} | true | true | accepted_and_approved |`, 'packet doc exact values');
}
passAssertion('all_19_build_runner_exact_values_present_and_approved');
...
passAssertion('future_command_status_blocked_until_build_runner_pre_run_guard_passes');
...
mustHave(packetDoc, 'This packet captures signed build-runner approval evidence **only**', 'packet doc');
passAssertion('packet_captures_signed_build_runner_approval_evidence_only');
...
mustHave(packetDoc, 'This packet does **not** pass the build-runner pre-run guard by itself', 'packet doc');
passAssertion('packet_does_not_pass_build_runner_pre_run_guard');
...
mustHave(packetDoc, 'build-runner pre-run guard pass', 'packet doc');
passAssertion('next_step_build_runner_pre_run_guard_or_scaffolding_build_packet');

mustHave(packetDoc, 'demo_ready_with_live_automation_disabled | preserved', 'packet doc');

for (const check of BUILD_RUNNER_CAPTURE_CHECKS) {
  mustHave(packetDoc, check, 'packet doc build runner capture checks');
  mustHave(packetDoc, `| ${check} | passed |`, 'packet doc build runner capture check status');
}
passAssertion('build_runner_signed_approval_capture_checks_all_10_checks_present');
passAssertion('build_runner_signed_approval_capture_checks_all_10_checks_passed');
...
for (const upstreamFixturePath of [buildRunnerTemplateFixturePath, runnerDesignFixturePath]) {
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be 07421c8');
}
...
if (fixture.future_command_status !== 'blocked_until_build_runner_pre_run_guard_passes') {
...
if (
  !Array.isArray(fixture.build_runner_signed_approval_capture_checks) ||
  fixture.build_runner_signed_approval_capture_checks.length !== 10
) {
  fail('fixture build_runner_signed_approval_capture_checks must contain 10 checks');
}
for (let i = 0; i < BUILD_RUNNER_CAPTURE_CHECKS.length; i += 1) {
  const check = fixture.build_runner_signed_approval_capture_checks[i];
  if (check.check !== i + 1) {
    fail(`fixture build_runner_signed_approval_capture_checks[${i}].check must be ${i + 1}`);
  }
  if (check.name !== BUILD_RUNNER_CAPTURE_CHECKS[i]) {
    fail(`fixture build_runner_signed_approval_capture_checks[${i}].name mismatch`);
  }
  if (check.status !== 'passed') {
    fail(`fixture build_runner_signed_approval_capture_checks[${i}].status must be passed`);
  }
}
...
if (upstreamTemplateFixture.build_runner_approval_capture_status !== 'not_captured') {
  fail('upstream build-runner template fixture build_runner_approval_capture_status must be not_captured');
}
...
mustHave(wrapper, 'local signed approval capture only', 'wrapper mode');
...
mustHave(
  wrapper,
  'future_command_status is blocked_until_build_runner_pre_run_guard_passes',
  'wrapper mode',
);
mustHave(wrapper, 'build-runner pre-run guard', 'wrapper mode');
...
if (!isExecutable(wrapperPath)) {
  fail(`signed build-runner capture dry-run wrapper is not executable: ${wrapperPath}`);
}
passAssertion('signed_build_runner_capture_dry_run_wrapper_present_and_safe');
...
if (REQUIRED_ASSERTIONS.length !== 80) {
  fail(`REQUIRED_ASSERTIONS must contain 80 items (found ${REQUIRED_ASSERTIONS.length})`);
}

console.log(
  `PASS: Native Workflow Fixture Capture Signed Build-Runner Approval verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
