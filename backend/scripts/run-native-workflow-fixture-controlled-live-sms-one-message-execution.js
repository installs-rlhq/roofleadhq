#!/usr/bin/env node
/**
 * Native Workflow Fixture — One-Message Controlled LIVE SMS Execution (Build 186; message binding fixed in Build 213).
 *
 ...
 *
 * Build 213 MESSAGE-BINDING FIX (root cause of the Build 212 content mismatch):
 *   - The outbound message body is no longer a hardcoded generic delivery-test string literal. It is
 *     BOUND to the approved selected variant text read from the current signed approval packet
 *     (signed_approval_capture_evidence.selected_variant_text). The approved variant source is explicit.
 *   - A FAIL-CLOSED guard blocks the send unless the resolved outbound body EXACTLY equals the approved
 *     selected variant text from the current signed approval packet, and additionally rejects the known
 *     stale generic delivery-test copy. No live send may proceed from stale/hardcoded generic copy.
 *   - resolveOutboundBody()/assertOutboundBodyMatchesApprovedVariant() are exported as PURE helpers so a
 *     local read-only verifier can prove the resolved outbound message equals R1 (and is not generic copy)
 *     WITHOUT any send, Twilio client, or network call. Requiring this module performs NO send and NO exit.
 *
...
// Explicit, verified approved-variant source: the current signed per-attempt approval packet.
const APPROVAL_PACKET_PATH = `${FIXTURE_DIR}/r1-sms-send-approval-build-211.json`;
...
// The stale generic delivery-test copy that caused the Build 212 content mismatch. Never send this.
const FORBIDDEN_STALE_GENERIC_BODY =
...
// -------------------------------------------------------------------------------------------------
// PURE HELPERS — message binding + fail-closed guard. No network, no Twilio, no env, no process.exit.
// Exported so a read-only verifier can prove the resolved outbound message equals the approved variant.
// -------------------------------------------------------------------------------------------------

// Resolve the approved selected variant from the current signed approval packet (explicit source).
function resolveApprovedVariant() {
  const res = readJsonAt(APPROVAL_PACKET_PATH);
  if (!res.ok) return { ok: false, reason: res.reason, text: null, label: null, variant: null, signed: false, granted: false };
  const ev = (res.value && res.value.signed_approval_capture_evidence) || {};
  const text = typeof ev.selected_variant_text === 'string' ? ev.selected_variant_text : null;
  return {
    ok: typeof text === 'string' && text.trim().length > 0,
    reason: typeof text === 'string' && text.trim().length > 0 ? null : 'approved_variant_text_missing_in_signed_approval_packet',
    text,
    label: ev.selected_variant_label || null,
    variant: ev.selected_message_variant || null,
    signed: ev.approval_signed === true,
    granted: ev.approval_granted === true
  };
}

// The outbound body is BOUND to the approved selected variant text (never a hardcoded generic literal).
function resolveOutboundBody() {
  const approved = resolveApprovedVariant();
  return approved.ok ? approved.text : null;
}

// Fail-closed guard: returns an array of block reasons (empty array == permitted).
function assertOutboundBodyMatchesApprovedVariant(body, approved) {
  const reasons = [];
  if (!approved || approved.ok !== true || typeof approved.text !== 'string' || approved.text.trim().length === 0) {
    reasons.push('approved_selected_variant_text_unavailable');
    return reasons; // cannot validate body against a missing approved variant => fail closed
  }
  if (approved.signed !== true || approved.granted !== true) reasons.push('approval_packet_not_signed_or_granted');
  if (typeof body !== 'string' || body.trim().length === 0) reasons.push('outbound_body_empty');
  if (typeof body === 'string' && body !== approved.text) reasons.push('outbound_body_does_not_match_approved_selected_variant');
  if (typeof body === 'string' && body === FORBIDDEN_STALE_GENERIC_BODY) reasons.push('outbound_body_is_stale_generic_delivery_test_copy');
  return reasons;
}
...
module.exports = {
  ...
  FORBIDDEN_STALE_GENERIC_BODY,
  resolveApprovedVariant,
  resolveOutboundBody,
  assertOutboundBodyMatchesApprovedVariant
};

// -------------------------------------------------------------------------------------------------
// LIVE EXECUTION — runs ONLY when invoked directly (require.main === module). Requiring is side-effect free.
// -------------------------------------------------------------------------------------------------
async function runMain() {
  const blockedReasons = [];
  function block(reason) { blockedReasons.push(reason); }
  function readJson(rel) {
    const res = readJsonAt(rel);
    if (!res.ok) { block(res.reason); return null; }
    return res.value;
  }
  ...
  // Build 213: bind outbound body to the approved selected variant and assert exact match (fail-closed).
  const approvedVariant = resolveApprovedVariant();
  if (!approvedVariant.ok) block('approved_selected_variant_unavailable:' + (approvedVariant.reason || 'unknown'));
  const resolvedBody = approvedVariant.ok ? approvedVariant.text : null;
  for (const reason of assertOutboundBodyMatchesApprovedVariant(resolvedBody, approvedVariant)) {
    block('message_binding_guard:' + reason);
  }
  ...
  async function performSend() {
    ...
    // Re-assert the message binding guard immediately before constructing the client (defense in depth).
    const reassert = assertOutboundBodyMatchesApprovedVariant(resolvedBody, approvedVariant);
    if (reassert.length > 0) { for (const r of reassert) block('message_binding_guard:' + r); return out; }
    ...
    try {
      const msg = await client.messages.create({
        ...
        body: resolvedBody // BOUND to the approved selected variant text (Build 213 fix)
      });
      out.smsSent = true;
  ...
  // EVIDENCE
  // -----------------------------------------------------------------------------------------------
  const finalPermitted = blockedReasons.length === 0;
  const finalDecision = finalPermitted
  ...
  const evidenceDoc = {
    ...
    // Message binding (Build 213) — names/labels/booleans only; the variant text is non-secret roofer copy.
    message_binding: {
      bound_to_approved_selected_variant: true,
      approved_variant_source_path: APPROVAL_PACKET_PATH,
      selected_variant: approvedVariant.variant || null,
      selected_variant_label: approvedVariant.label || null,
      outbound_body_equals_approved_selected_variant: assertOutboundBodyMatchesApprovedVariant(resolvedBody, approvedVariant).length === 0,
      outbound_body_is_stale_generic_delivery_test_copy: resolvedBody === FORBIDDEN_STALE_GENERIC_BODY,
      hardcoded_generic_body_used: false
    },

    // Pre-flight outcome
    pre_flight_permitted: finalPermitted,
  ...
  console.log('MESSAGE BINDING: bound_to_approved_selected_variant=' + evidenceDoc.message_binding.bound_to_approved_selected_variant +
    ' | outbound_body_equals_approved_selected_variant=' + evidenceDoc.message_binding.outbound_body_equals_approved_selected_variant);
  console.log('PRE-FLIGHT: ' + (finalPermitted ? 'PERMITTED' : 'BLOCKED'));
  if (!finalPermitted) {
