...
const SOURCE_OF_TRUTH_COMMIT = '640df59';
...
const SCENARIO_REQUIRED_FIELDS = [
  ...
  'scenario_group',
  'scenario_name',
  'approved_boundary_required',
  'service_mode_required',
  'test_account_reference_required',
  'input_fixture_required',
  'expected_result',
  'evidence_required',
  'stop_condition_required',
  'reviewer_initials_required',
  'execution_status',
  'pass_fail_status',
];

const SCAFFOLDING_BUILD_CHECKS = [
  'source_of_truth_commit_640df59_confirmed',
  'build_runner_pre_run_guard_packet_present_and_passed',
  ...
  'runner_command_path_created_fail_closed',
  'manifest_created_with_30_scenarios',
  'all_scenarios_execution_status_not_run',
  'all_scenarios_pass_fail_status_not_captured',
...
const REQUIRED_ASSERTIONS = [
  ...
  'manifest_fixture_valid_json',
  'upstream_build_runner_pre_run_guard_fixture_present',
  ...
  'source_of_truth_commit_640df59_referenced',
  'build_runner_pre_run_guard_commit_640df59_referenced',
  ...
  'build_runner_pre_run_guard_packet_referenced',
  ...
  'current_runner_gap_status_scaffolding_created_but_execution_not_approved_not_run',
  ...
  'future_command_status_blocked_until_runner_execution_exact_approval_captured_and_execution_pre_run_guard_passes',
  ...
  'manifest_group_counts_correct',
  'all_scenario_required_fields_present',
  'all_scenarios_execution_status_not_run',
  'all_scenarios_pass_fail_status_not_captured',
  'packet_is_scaffolding_build_only',
  ...
  'next_step_runner_execution_exact_approval_template_or_stop_review_not_execution',
  'scaffolding_build_checks_all_20_checks_present',
  'scaffolding_build_checks_all_20_checks_passed',
  ...
  'runner_command_path_fail_closed',
  'runner_contains_blocked_no_go_language',
  'runner_contains_no_live_execution_approvals',
  'runner_contains_no_credential_values',
  'runner_contains_no_production_supabase_activation',
  'runner_contains_no_sms_email_call_calendar_activation',
  'wrapper_does_not_invoke_runner',
  'demo_ready_with_live_automation_disabled_preserved',
  'full_safe_readiness_lane_preserved',
  'docs_and_context_wiring_present',
  'scaffolding_build_dry_run_wrapper_present_and_safe',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_ACTUAL_EXTERNAL_SANDBOX_30_SCENARIO_RUNNER_SCAFFOLDING_BUILD.md',
  'NATIVE_WORKFLOW_FIXTURE_ACTUAL_EXTERNAL_SANDBOX_30_SCENARIO_RUNNER_SCAFFOLDING_BUILD_NO_GO_REVIEW.md',
  'run-native-workflow-fixture-actual-external-sandbox-30-scenario-runner-scaffolding-build-dry-run.sh',
  'verify-native-workflow-fixture-actual-external-sandbox-30-scenario-runner-scaffolding-build-readonly.js',
  'actual-external-sandbox-30-scenario-runner-scaffolding-build.json',
  'actual-external-sandbox-30-scenario-validation-manifest.json',
  'Native Workflow Fixture Actual External/Sandbox 30-Scenario Runner Scaffolding Build',
  'native workflow fixture actual external sandbox 30 scenario runner scaffolding build',
  'runner scaffolding build',
];
...
const RUNNER_UNSAFE_STRINGS = [
  'runner_execution_approval_status: granted',
  'external_calls_approval_status: granted',
  'credentials_access_approval_status: granted',
...
mustHave(
  packetDoc,
  'fake-data/local-only/read-only/dry-run-only/review-only/runner-scaffolding-build-only/non-executing',
  'packet doc',
);
mustHave(
  noGoReviewDoc,
...
passAssertion('source_of_truth_commit_640df59_referenced');

mustHave(packetDoc, BUILD_RUNNER_PRE_RUN_GUARD_COMMIT, 'packet doc');
mustHave(noGoReviewDoc, BUILD_RUNNER_PRE_RUN_GUARD_COMMIT, 'no-go review');
passAssertion('build_runner_pre_run_guard_commit_640df59_referenced');
...
mustHave(packetDoc, 'build-runner-pre-run-guard', 'packet doc');
passAssertion('build_runner_pre_run_guard_packet_referenced');
...
passAssertion('current_runner_gap_status_scaffolding_created_but_execution_not_approved_not_run');
...
mustHave(packetDoc, 'This builds **local runner scaffolding only**', 'packet doc');
passAssertion('packet_is_scaffolding_build_only');
...
mustHave(packetDoc, 'exact runner-execution approval template', 'packet doc');
passAssertion('next_step_runner_execution_exact_approval_template_or_stop_review_not_execution');

mustHave(packetDoc, 'demo_ready_with_live_automation_disabled | preserved', 'packet doc');

for (const check of SCAFFOLDING_BUILD_CHECKS) {
  mustHave(packetDoc, check, 'packet doc scaffolding build checks');
  mustHave(packetDoc, `| ${check} | passed |`, 'packet doc scaffolding build check status');
}
passAssertion('scaffolding_build_checks_all_20_checks_present');
passAssertion('scaffolding_build_checks_all_20_checks_passed');
...
for (const upstreamFixturePath of [
  upstreamPreRunGuardFixturePath,
...
passAssertion('upstream_build_runner_pre_run_guard_fixture_present');
...
const upstreamPreRunGuardFixture = readJson(upstreamPreRunGuardFixturePath);
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be 640df59');
}
if (fixture.build_runner_pre_run_guard_commit !== BUILD_RUNNER_PRE_RUN_GUARD_COMMIT) {
  fail('fixture build_runner_pre_run_guard_commit must be 640df59');
}
...
if (
  fixture.current_runner_gap_status !== 'scaffolding_created_but_execution_not_approved_not_run'
) {
...
if (fixture.total_manifest_scenarios_count !== 30) {
  fail('fixture total_manifest_scenarios_count must be 30');
}
...
if (!Array.isArray(fixture.scaffolding_build_checks) || fixture.scaffolding_build_checks.length !== 20) {
  fail('fixture scaffolding_build_checks must contain 20 checks');
}
for (let i = 0; i < SCAFFOLDING_BUILD_CHECKS.length; i += 1) {
  const check = fixture.scaffolding_build_checks[i];
  if (check.check !== i + 1) {
    fail(`fixture scaffolding_build_checks[${i}].check must be ${i + 1}`);
  }
  if (check.name !== SCAFFOLDING_BUILD_CHECKS[i]) {
    fail(`fixture scaffolding_build_checks[${i}].name mismatch`);
  }
  if (check.status !== 'passed') {
    fail(`fixture scaffolding_build_checks[${i}].status must be passed`);
  }
}

if (!Array.isArray(manifest.scenarios) || manifest.scenarios.length !== 30) {
  fail('manifest must contain exactly 30 scenarios');
}
...
for (const scenario of manifest.scenarios) {
  for (const field of SCENARIO_REQUIRED_FIELDS) {
    if (!(field in scenario)) {
      fail(`manifest scenario ${scenario.scenario_id || 'unknown'} missing field ${field}`);
    }
  }
  if (scenario.execution_status !== 'not_run') {
    fail(`manifest scenario ${scenario.scenario_id} execution_status must be not_run`);
  }
  if (scenario.pass_fail_status !== 'not_captured') {
    fail(`manifest scenario ${scenario.scenario_id} pass_fail_status must be not_captured`);
  }
  groupCounts[scenario.scenario_group] = (groupCounts[scenario.scenario_group] || 0) + 1;
}

for (const [group, expectedCount] of Object.entries(MANIFEST_GROUP_COUNTS)) {
  if (groupCounts[group] !== expectedCount) {
    fail(`manifest group ${group} must have ${expectedCount} scenarios (found ${groupCounts[group] || 0})`);
  }
}
passAssertion('manifest_group_counts_correct');
passAssertion('all_scenario_required_fields_present');
passAssertion('all_scenarios_execution_status_not_run');
passAssertion('all_scenarios_pass_fail_status_not_captured');

if (manifest.actual_30_scenario_external_validation_captured_count !== 0) {
  fail('manifest actual_30_scenario_external_validation_captured_count must be 0');
}
if (manifest.actual_30_scenario_external_validation_passed_count !== 0) {
  fail('manifest actual_30_scenario_external_validation_passed_count must be 0');
}
if (manifest.actual_30_scenario_external_validation_missing_count !== 30) {
  fail('manifest actual_30_scenario_external_validation_missing_count must be 30');
}

if (upstreamPreRunGuardFixture.build_runner_pre_run_guard_status !== 'passed') {
  fail('upstream pre-run guard fixture build_runner_pre_run_guard_status must be passed');
}
if (upstreamPreRunGuardFixture.future_command_status !== 'ready_for_build_runner_scaffolding_packet_review_only') {
  fail('upstream pre-run guard fixture future_command_status mismatch');
}
...
passAssertion('manifest_fixture_valid_json');
...
mustHave(runner, 'FAIL-CLOSED', 'runner');
mustHave(runner, 'BLOCKED', 'runner');
mustHave(runner, 'NO-GO', 'runner');
mustHave(runner, 'exit 1', 'runner');
mustHave(runner, 'not_granted', 'runner');
mustHave(runner, 'created_fail_closed_not_approved_to_run', 'runner');
passAssertion('runner_command_path_fail_closed');
passAssertion('runner_contains_blocked_no_go_language');

for (const unsafe of RUNNER_UNSAFE_STRINGS) {
  if (runner.includes(unsafe)) {
    fail(`unsafe string ${unsafe} found in runner`);
  }
}
passAssertion('runner_contains_no_live_execution_approvals');
passAssertion('runner_contains_no_credential_values');
passAssertion('runner_contains_no_production_supabase_activation');
passAssertion('runner_contains_no_sms_email_call_calendar_activation');

mustNotHave(
  wrapper,
  'bash scripts/run-native-workflow-fixture-actual-external-sandbox-30-scenario-validation.sh',
  'wrapper must not invoke runner',
);
...
mustHave(aggregate, 'Native Workflow Fixture Actual External/Sandbox 30-Scenario Runner Scaffolding Build', 'aggregate readiness');
...
mustHave(verifierIndex, manifestPath, 'verifier index');
...
mustHave(wrapper, 'local runner scaffolding build only', 'wrapper mode');
...
mustHave(wrapper, 'runner_scaffolding_build_status is built_review_only', 'wrapper mode');
...
mustHave(
  wrapper,
  'future_command_status is blocked_until_runner_execution_exact_approval_captured_and_execution_pre_run_guard_passes',
  'wrapper mode',
);
mustHave(wrapper, 'runner-execution exact approval template or stop/review', 'wrapper mode');
...
if (!isExecutable(wrapperPath)) {
  fail(`scaffolding build dry-run wrapper is not executable: ${wrapperPath}`);
}
passAssertion('scaffolding_build_dry_run_wrapper_present_and_safe');

mustHave(packetDoc, fixturePath, 'packet doc');
mustHave(packetDoc, manifestPath, 'packet doc');
...
console.log(
  `PASS: Native Workflow Fixture Actual External/Sandbox 30-Scenario Runner Scaffolding Build verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
