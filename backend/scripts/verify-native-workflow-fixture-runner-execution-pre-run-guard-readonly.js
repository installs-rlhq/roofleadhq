...
const packetDocPath = 'docs/NATIVE_WORKFLOW_FIXTURE_RUNNER_EXECUTION_PRE_RUN_GUARD.md';
...
const SOURCE_OF_TRUTH_COMMIT = 'bb0bc14';
...
const CAPTURE_ACCEPTED_EXACT_VALUE_FIELDS = [
...
const CAPTURE_APPROVED_EXACT_VALUES = {
  ...
  exact_structured_evidence_output_path_pattern:
    'backend/fixtures/native-workflow-demo-roofer/actual-external-sandbox-30-scenario-validation-result-{YYYY-MM-DD-HHMMSS}.json',
  runner_execution_approval_capture_status: 'approved_for_capture',
...
const PRE_RUN_GUARD_CHECKS = [
  'source_of_truth_commit_bb0bc14_confirmed',
  'signed_runner_execution_approval_capture_packet_present',
  'signed_approval_timestamp_present',
  'approval_scope_one_time_30_scenario_validation_only',
  'exact_working_directory_matches_root_roofleadhq',
  'exact_command_matches_approved_runner_command',
  'exact_runner_path_exists',
  'exact_runner_path_executable',
  'exact_manifest_path_exists',
  'exact_scenario_count_30',
  'all_24_exact_values_required_accepted_approved',
  'runner_execution_approval_granted_scoped_one_time_pending_pre_run_guard',
  'external_sandbox_calls_granted_scoped_test_mode_only',
  'credentials_access_granted_scoped_test_mode_only_no_secret_logging',
  'test_account_use_granted_scoped_test_accounts_only',
  ...
  'runner_not_invoked_by_this_packet',
  'no_external_calls_made_by_this_packet',
  'no_credentials_accessed_by_this_packet',
  'no_production_data_accessed_by_this_packet',
  'no_real_contact_or_sms_email_calls_calendar_booking_by_this_packet',
...
const REQUIRED_ASSERTIONS = [
  ...
  'source_of_truth_commit_bb0bc14_referenced',
  ...
  'runner_command_path_status_created_fail_closed_pending_exact_approved_execution_command',
  'manifest_path_exists',
  'total_manifest_scenarios_count_30',
  'runner_execution_approval_status_granted_scoped_one_time_pending_execution_pre_run_guard',
  ...
  'packet_is_execution_pre_run_guard_only',
  ...
  'next_step_exact_approved_runner_execution_command_in_terminal_1_or_stop_review',
  'execution_pre_run_guard_checks_all_30_checks_present',
  'execution_pre_run_guard_checks_all_30_checks_passed',
  'demo_ready_with_live_automation_disabled_preserved',
  'full_safe_readiness_lane_preserved',
  'docs_and_context_wiring_present',
  'runner_execution_pre_run_guard_dry_run_wrapper_present_and_safe',
...
const PACKET_REFS = [
  'NATIVE_WORKFLOW_FIXTURE_RUNNER_EXECUTION_PRE_RUN_GUARD.md',
  'NATIVE_WORKFLOW_FIXTURE_RUNNER_EXECUTION_PRE_RUN_GUARD_NO_GO_REVIEW.md',
  'run-native-workflow-fixture-runner-execution-pre-run-guard-dry-run.sh',
  'verify-native-workflow-fixture-runner-execution-pre-run-guard-readonly.js',
  'runner-execution-pre-run-guard.json',
  'Native Workflow Fixture Runner-Execution Pre-Run Guard',
  'native workflow fixture runner execution pre run guard',
  'runner execution pre run guard',
];
...
mustHave(
  packetDoc,
  'fake-data/local-only/read-only/dry-run-only/review-only/execution-pre-run-guard-only/non-executing',
  'packet doc',
);
mustHave(
  noGoReviewDoc,
...
passAssertion('source_of_truth_commit_bb0bc14_referenced');
...
mustHave(packetDoc, 'runner-execution-exact-approval-template', 'packet doc');
...
mustHave(packetDoc, 'all_24_exact_values_required_accepted_approved', 'packet doc');
passAssertion('all_24_exact_values_required_accepted_approved');
...
passAssertion('runner_command_path_status_created_fail_closed_pending_exact_approved_execution_command');
...
passAssertion('runner_execution_approval_status_granted_scoped_one_time_pending_execution_pre_run_guard');
...
mustHave(packetDoc, 'This is an **execution pre-run guard only**', 'packet doc');
passAssertion('packet_is_execution_pre_run_guard_only');
...
mustHave(packetDoc, 'exact approved runner command execution in Terminal 1', 'packet doc');
passAssertion('next_step_exact_approved_runner_execution_command_in_terminal_1_or_stop_review');
...
passAssertion('execution_pre_run_guard_checks_all_30_checks_present');
passAssertion('execution_pre_run_guard_checks_all_30_checks_passed');
...
for (const upstreamFixturePath of [
  ...
  upstreamRunnerExecutionTemplateFixturePath,
...
const upstreamTemplateFixture = readJson(upstreamRunnerExecutionTemplateFixturePath);
...
if (fixture.source_of_truth_commit !== SOURCE_OF_TRUTH_COMMIT) {
  fail('fixture source_of_truth_commit must be bb0bc14');
}
if (fixture.capture_signed_runner_execution_approval_commit !== CAPTURE_SIGNED_RUNNER_EXECUTION_APPROVAL_COMMIT) {
  fail('fixture capture_signed_runner_execution_approval_commit must be bb0bc14');
}
if (fixture.runner_execution_exact_approval_template_commit !== RUNNER_EXECUTION_TEMPLATE_COMMIT) {
  fail('fixture runner_execution_exact_approval_template_commit must be 67393ed');
}
...
if (fixture.runner_execution_approval_capture_status !== 'captured') {
  fail('fixture runner_execution_approval_capture_status must be captured');
}
if (fixture.runner_execution_jason_signed_approval_status !== 'signed') {
  fail('fixture runner_execution_jason_signed_approval_status must be signed');
}
...
if (fixture.runner_execution_approval_status !== 'granted_scoped_one_time_pending_execution_pre_run_guard') {
  fail('fixture runner_execution_approval_status mismatch');
}
...
if (
  !Array.isArray(fixture.execution_pre_run_guard_checks) ||
  fixture.execution_pre_run_guard_checks.length !== 30
) {
  fail('fixture execution_pre_run_guard_checks must contain 30 checks');
}
for (let i = 0; i < PRE_RUN_GUARD_CHECKS.length; i += 1) {
  const check = fixture.execution_pre_run_guard_checks[i];
  if (check.check !== i + 1) {
    fail(`fixture execution_pre_run_guard_checks[${i}].check must be ${i + 1}`);
  }
  if (check.name !== PRE_RUN_GUARD_CHECKS[i]) {
    fail(`fixture execution_pre_run_guard_checks[${i}].name mismatch`);
  }
  if (check.status !== 'passed') {
    fail(`fixture execution_pre_run_guard_checks[${i}].status must be passed`);
  }
}

if (upstreamCaptureFixture.runner_execution_approval_capture_status !== 'captured') {
  fail('upstream capture fixture runner_execution_approval_capture_status must be captured');
}
if (upstreamCaptureFixture.runner_execution_jason_signed_approval_status !== 'signed') {
  fail('upstream capture fixture runner_execution_jason_signed_approval_status must be signed');
}
if (upstreamCaptureFixture.runner_execution_exact_values_required_count !== 24) {
  fail('upstream capture fixture runner_execution_exact_values_required_count must be 24');
}
if (upstreamCaptureFixture.runner_execution_exact_values_accepted_count !== 24) {
  fail('upstream capture fixture runner_execution_exact_values_accepted_count must be 24');
}
if (upstreamCaptureFixture.runner_execution_exact_values_approved_count !== 24) {
  fail('upstream capture fixture runner_execution_exact_values_approved_count must be 24');
}
if (
  upstreamCaptureFixture.future_command_status !==
  'blocked_until_runner_execution_pre_run_guard_passes'
) {
  fail('upstream capture fixture future_command_status must be blocked_until_runner_execution_pre_run_guard_passes');
}
if (!upstreamCaptureFixture.accepted_exact_values || typeof upstreamCaptureFixture.accepted_exact_values !== 'object') {
  fail('upstream capture fixture accepted_exact_values object missing');
}
if (!upstreamCaptureFixture.approved_exact_values || typeof upstreamCaptureFixture.approved_exact_values !== 'object') {
  fail('upstream capture fixture approved_exact_values object missing');
}
for (const field of CAPTURE_ACCEPTED_EXACT_VALUE_FIELDS) {
  if (!(field in upstreamCaptureFixture.accepted_exact_values)) {
    fail(`upstream capture fixture accepted_exact_values missing ${field}`);
  }
  if (upstreamCaptureFixture.accepted_exact_values[field] !== CAPTURE_APPROVED_EXACT_VALUES[field]) {
    fail(`upstream capture fixture accepted_exact_values.${field} mismatch`);
  }
  if (!(field in upstreamCaptureFixture.approved_exact_values)) {
    fail(`upstream capture fixture approved_exact_values missing ${field}`);
  }
  if (upstreamCaptureFixture.approved_exact_values[field] !== CAPTURE_APPROVED_EXACT_VALUES[field]) {
    fail(`upstream capture fixture approved_exact_values.${field} mismatch`);
  }
}

if (upstreamTemplateFixture.source_of_truth_commit !== '145bf15') {
  fail('upstream runner-execution template fixture source_of_truth_commit must be 145bf15');
}
if (upstreamTemplateFixture.runner_execution_approval_capture_status !== 'not_captured') {
  fail('upstream runner-execution template fixture runner_execution_approval_capture_status must be not_captured');
}
...
mustHave(aggregate, 'Native Workflow Fixture Runner-Execution Pre-Run Guard', 'aggregate readiness');
...
mustHave(wrapper, 'local execution pre-run guard only', 'wrapper mode');
...
mustHave(wrapper, RUNNER_EXECUTION_TEMPLATE_COMMIT, 'wrapper Build 104');
mustHave(wrapper, RUNNER_SCAFFOLDING_BUILD_COMMIT, 'wrapper Build 103');
mustHave(
  wrapper,
  'future_command_status is ready_for_exact_approved_runner_execution_command_review_only',
  'wrapper mode',
);
mustHave(wrapper, 'exact approved runner command execution in Terminal 1', 'wrapper mode');
...
if (!isExecutable(wrapperPath)) {
  fail(`runner-execution pre-run guard dry-run wrapper is not executable: ${wrapperPath}`);
}
passAssertion('runner_execution_pre_run_guard_dry_run_wrapper_present_and_safe');
...
console.log(
  `PASS: Native Workflow Fixture Runner-Execution Pre-Run Guard verified (${REQUIRED_ASSERTIONS.length} assertions).`,
);
console.log(
