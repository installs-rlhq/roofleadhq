...
const doc = read('docs/ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_ARCHIVE_ACCEPTANCE.md');
const wrapper = read('scripts/accept-first-roofer-manual-setup-session-archive-dry-run.sh');
...
if (!aggregate.includes('verify-roofer-dry-run-first-roofer-manual-setup-session-archive-acceptance-readonly.js')) fail('aggregate missing session archive acceptance verifier');
if (!index.includes('ROOFER_DRY_RUN_FIRST_ROOFER_MANUAL_SETUP_SESSION_ARCHIVE_ACCEPTANCE.md')) fail('index missing session archive acceptance packet');
if (!firstPaid.includes('First Roofer Manual Setup Session Archive Acceptance Packet')) fail('first-paid context missing session archive acceptance packet');
if (!roofer.includes('First Roofer Manual Setup Session Archive Acceptance Packet')) fail('roofer context missing session archive acceptance packet');

console.log('PASS: first-roofer manual setup session archive acceptance verifier passed.');
